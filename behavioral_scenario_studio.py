    ...
    if not events:
        ...
        if not interactions:
            ...
            with st.form(
                ...
                st.markdown("**Participant state before and after the interaction**")
                st.caption(
                    "Record these as separate time-point observations: use "
                    "BASELINE for the participant's before state and IMMEDIATE "
                    "for the participant's after state. Use participant-reported "
                    "wording or directly observed, concrete descriptions only. "
                    "Do not diagnose, label, or infer a clinical or psychological condition."
                )

                before_state = st.text_area(
                    "Before state (record at BASELINE)",
                    height=80,
                    placeholder=(
                        "At BASELINE, describe what the participant reports or "
                        "what is directly observable before the outreach interaction."
                    ),
                )

                after_state = st.text_area(
                    "After state (record at IMMEDIATE)",
                    height=80,
                    placeholder=(
                        "At IMMEDIATE, describe what the participant reports or "
                        "what is directly observable after the outreach interaction."
                    ),
                )
