...
# - Set OUTREACH_DATABASE_URL to a persistent database URL in deployment.
# - SQLite is retained as the local-development fallback.
# - The application never expires or deletes registered accounts automatically.
# - A hosting volume/database with backups is still required for true disaster recovery.
DATABASE_URL = os.getenv(
    "OUTREACH_DATABASE_URL",
    "sqlite:///ninolades_outreach_lab.db"
).strip() or "sqlite:///ninolades_outreach_lab.db"
...
if DATABASE_URL.startswith("sqlite:///:memory:"):
    raise RuntimeError(
        "OUTREACH_DATABASE_URL points to an in-memory SQLite database. "
...
def _normalize_username(username: str) -> str:
    """Normalize usernames consistently at registration and login.

    Usernames are case-insensitive and surrounding whitespace is ignored.
    Passwords are intentionally *not* normalized because whitespace and case
    may be meaningful parts of a password.
    """
    return str(username or "").strip().casefold()


def _query_param_text(value) -> str:
    """Return one query-parameter value across supported Streamlit versions."""
...
def _verify_password(password: str, stored_hash: str) -> bool:
    """Verify a password hash without ever raising on malformed database data."""
...
auth_db = db_session()
raw_auth_token = _query_param_text(st.query_params.get("auth_session"))
...
if authenticated_user:
    ...
    with login_tab:
        ...
        if login_submitted:
            # Look up the same canonical username representation used during
            # registration. Do not strip or otherwise alter the password.
            username = _normalize_username(login_username)
            password = login_password if isinstance(login_password, str) else ""
...
with header_col2:
    ...
    if st.button("Log out", use_container_width=True):
        current_token = (
            ...
            or _query_param_text(st.query_params.get("auth_session"))
        )
