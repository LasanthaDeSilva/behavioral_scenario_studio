...
# Streamlit button widgets are action triggers, not durable workspace state.
# Persisting their custom keys and restoring them before the button is created
# causes StreamlitValueAssignmentNotAllowedError on the next rerun.  Keep all
# existing user-facing state persistence intact, but exclude only the custom
# action-widget keys used by this application.
_NON_PERSISTENT_WIDGET_KEY_PREFIXES = (
    "edit_saved_event_",
    "del_prof_",
    "del_rlog_",
    "obs_btn_",
...
def _is_non_persistent_widget_key(key) -> bool:
    key_text = str(key)
    return any(
        key_text.startswith(prefix)
        for prefix in _NON_PERSISTENT_WIDGET_KEY_PREFIXES
    )
...
def _persistent_state_snapshot():
    ...
    for key, value in st.session_state.items():
        if (
            ...
            or _is_non_persistent_widget_key(key)
        ):
            continue
...
def restore_persistent_state(db, user_id: str):
    ...
    for key, value in payload.items():
        if (
            ...
            and not _is_non_persistent_widget_key(key)
        ):
