...
# Live ambient acoustic capture migrations (additive; existing records remain compatible).
for _table, _column, _sql_type in (
    ("events", "ambient_noise_classification", "VARCHAR"),
    ("events", "ambient_noise_db", "FLOAT"),
    ("rapid_state_logs", "ambient_noise_classification", "VARCHAR"),
    ("rapid_state_logs", "ambient_noise_db", "FLOAT"),
):
    try:
        with engine.connect() as conn:
            conn.execute(text(f"ALTER TABLE {_table} ADD COLUMN {_column} {_sql_type}"))
            conn.commit()
    except Exception:
        pass
...
_AUDIO_STATE_LOCK = threading.RLock()

def classify_ambient_noise(db_value) -> str:
    try:
...
def get_live_ambient_state():
    with _AUDIO_STATE_LOCK:
        db_value = st.session_state.get("live_ambient_db")
        status = st.session_state.get("live_ambient_status", "inactive")
        classification = classify_ambient_noise(db_value)
        active = status == "active" and db_value is not None
        return db_value, classification, active

def render_live_audio_tracker():
    # Browser microphone samples are relative digital levels, not calibrated SPL.
    # The browser sends a stable, explicitly labelled calibrated estimate for UI use.
    component_html = r"""
    <div id="ambient-root" style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#111114;color:#f5f5f7;border:1px solid #28282e;border-radius:14px;padding:14px 16px;box-sizing:border-box">
      <button id="authorize" style="width:100%;border:1px solid rgba(255,255,255,.16);border-radius:10px;background:linear-gradient(135deg,#1b1b20,#111114);color:#fff;padding:12px 14px;font-size:14px;font-weight:600;cursor:pointer">🎙️ Authorize Live Audio Tracking</button>
      <div id="status" style="margin-top:9px;color:#a1a1aa;font-size:12px">Microphone tracking is inactive.</div>
    </div>
    <script>
    (() => {
      const btn=document.getElementById('authorize'), status=document.getElementById('status');
      let ctx, source, analyser, stream, timer, data;
      const post=(payload)=>window.parent.postMessage({type:'ninolades-live-ambient-audio',payload}, '*');

      // Soft perceptual weighting: suppress very low-frequency rumble and emphasize speech.
      function perceptualRms(freqData, sampleRate) {
        const n=freqData.length, nyquist=sampleRate/2;
        let weighted=0, weightSum=0;
        for(let i=1;i<n;i++){
          const f=i*nyquist/n;
          let w;
          if(f<60) w=0.05;
          else if(f<120) w=0.22;
          else if(f<250) w=0.55;
          else if(f<800) w=0.85;
          else if(f<=4000) w=1.35;
          else if(f<=8000) w=0.95;
          else w=0.55;
          weighted += freqData[i]*freqData[i]*w;
          weightSum += w;
        }
        return Math.sqrt(weighted/Math.max(weightSum,1));
      }
      ...
      function analyse() {
        ...
        const pcmRms=Math.sqrt(sum/data.length);
        const spectrum=new Float32Array(analyser.frequencyBinCount);
        analyser.getFloatFrequencyData(spectrum);
        // Convert dBFS spectrum back to linear magnitude for soft perceptual weighting.
        const linear=new Float32Array(spectrum.length);
        for(let i=0;i<spectrum.length;i++) linear[i]=Math.pow(10,spectrum[i]/20);
        const perceptual=perceptualRms(linear,ctx.sampleRate);
        const rms=Math.max(1e-5, Math.max(pcmRms, perceptual));
        const relativeDb=20*Math.log10(rms);
        // Device-dependent estimate: reference offset maps typical speech-like browser levels
        // into an intelligible UI range. This is not a standards-traceable SPL measurement.
        const estimatedDb=Math.max(0, Math.min(110, relativeDb + 94));
        status.textContent=`Tracking active · ${estimatedDb.toFixed(1)} dB estimate`;
        post({status:'active', db:Number(estimatedDb.toFixed(2)), timestamp:Date.now()});
      }
      ...
      btn.addEventListener('click', async () => {
        try {
          ...
          btn.textContent='🎙️ Live Audio Tracking Active';
          btn.disabled=true;
          analyse();
          timer=setInterval(analyse,3000);
        } catch(err) {
          status.textContent='Microphone unavailable or permission was not granted.';
          post({status:'inactive', db:null, timestamp:Date.now()});
        }
      });
      window.addEventListener('beforeunload',()=>{if(timer)clearInterval(timer);if(stream)stream.getTracks().forEach(t=>t.stop());if(ctx)ctx.close();});
    })();
    </script>
    """
    components.html(component_html, height=118)

    # Parent listener persists only the latest scalar in browser storage and query params.
    # Python consumes it on normal Streamlit reruns; no audio is uploaded or recorded.
    components.html(r"""
    <script>
    (() => {
      const key='ninolades_live_ambient_state';
      try {
        const existing=window.parent.localStorage.getItem(key);
        if(existing) {
          const e=JSON.parse(existing);
          const u=new URL(window.parent.location.href);
          if(e && e.status==='active' && Number.isFinite(e.db) && u.searchParams.get('ambient_db')!==String(e.db)) {
            // Keep latest value available to Python on the next ordinary rerun without forcing navigation.
          }
        }
      } catch(e) {}
      window.parent.addEventListener('message',(event)=>{
        const m=event.data;
        if(!m || m.type!=='ninolades-live-ambient-audio') return;
        const p=m.payload||{};
        try { window.parent.localStorage.setItem(key,JSON.stringify(p)); } catch(e) {}
        const u=new URL(window.parent.location.href);
        if(p.status==='active' && Number.isFinite(p.db)) {
          u.searchParams.set('ambient_db',String(p.db));
          u.searchParams.set('ambient_status','active');
        } else {
          u.searchParams.delete('ambient_db'); u.searchParams.set('ambient_status','inactive');
        }
    ...
    qp_db = st.query_params.get("ambient_db")
    qp_status = st.query_params.get("ambient_status")
    with _AUDIO_STATE_LOCK:
        if qp_db is not None:
            try:
                st.session_state["live_ambient_db"] = float(qp_db)
            except (TypeError, ValueError):
                pass
        if qp_status:
            st.session_state["live_ambient_status"] = qp_status
...
def create_event(
    ...
    event = Event(
        ...
        ambient_noise_classification=get_live_ambient_state()[1],
        ambient_noise_db=get_live_ambient_state()[0],
...
# ============================================================
    ...
    if not events:
        ...
        render_html('<div class="section-heading">Live ambient acoustic tracking</div>')
        render_live_audio_tracker()
        live_db, live_noise_classification, live_active = get_live_ambient_state()
        meter_text = (
            f"🎙️ Live Ambient Meter: {live_db:.1f} dB — Tracking Active"
            if live_active and live_db is not None
            else "🎙️ Live Ambient Meter: awaiting authorization — Moderate Noise fallback"
        )
        st.caption(meter_text)
        ...
        with qc1:
            ...
            live_db, live_noise_classification, live_active = get_live_ambient_state()
            # Real-time engine drives the active selection whenever tracking is active.
            if live_active:
                st.session_state["q_noise"] = live_noise_classification
            elif "q_noise" not in st.session_state:
...
# ============================================================
    ...
    render_live_audio_tracker()
    _live_db, _live_classification, _live_active = get_live_ambient_state()
    st.caption(
        f"🎙️ Live Ambient Meter: {_live_db:.1f} dB — Tracking Active"
        if _live_active and _live_db is not None
        else "🎙️ Live Ambient Meter: awaiting authorization — Moderate Noise fallback"
    )
    ...
    if not events:
        ...
        if st.button("Log as New Participant", type="primary", use_container_width=True):
            ...
            try:
                ...
                db.rollback()
                st.error(f"State could not be saved safely: {exc}")
