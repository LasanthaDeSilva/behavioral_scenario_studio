...
def get_db_engine():
    ...
    # Streamlit reruns the script frequently and multiple user sessions can
    # execute it concurrently.  A default SQLAlchemy QueuePool can therefore
    # become exhausted when a session retains a connection across a rerun.
    # Use NullPool for SQLite (which is also friendlier to SQLite's single-file
    # locking model) and a bounded, health-checked pool for server databases.
    if DATABASE_URL.startswith("sqlite"):
...
def _persistent_state_snapshot():
    protected = {
        "user_session_token", "authenticated_user_id",
        "authenticated_username", "auth_token", "session_verified_locally",
        "ls_synced", "_trigger_ls_update", "confirm_clean_memory",
        "confirm_clean_memory_checkbox", "persistent_state_restored",
...
def restore_persistent_state(db, user_id: str):
    ...
    for key, value in payload.items():
        if key not in protected and not str(key).startswith("_"):
...
def app_rerun():
    """Persist current workspace state, release the DB connection, then rerun."""
    try:
        persist_app_state(db, USER_ACCOUNT_ID)
    except Exception:
        db.rollback()
    finally:
        # Do not retain a checked-out connection across Streamlit reruns.
        # This is essential when several browser sessions share the same
        # SQLAlchemy engine on Streamlit Cloud.
        db.close()
    st.rerun()
...
def _set_analysis_result(key: str, event_id: str, payload: dict):
    """Store an analysis together with the event it was generated for.

    This prevents a valid result from one program being silently displayed
    after the user switches to a different program.
    """
    st.session_state[key] = payload
    st.session_state[f"{key}_event_id"] = event_id


def _analysis_result_for_event(key: str, event_id: str):
    """Return an analysis only when it belongs to the selected event."""
    if st.session_state.get(f"{key}_event_id") != event_id:
        return None
...
Based on these parameters, provide model-generated, non-clinical estimates of plausible aggregate effects. These are hypotheses, not measurements, and should not be presented as physiological, neurological, or validated psychological scores. Do not claim individual-level prediction or causation.
Provide cautious, realistic estimates for:
...
Model plausible differences among the supplied hypothetical trait profiles. Treat HEXACO scores as descriptive trait inputs, not diagnoses or measurements of a real person. Do not infer protected attributes, clinical states, or deterministic behavior. Use established personality/behavior concepts cautiously and report uncertainty; these are scenario hypotheses, not individual-level predictions.
"""
    return run_gemini(
        client=client,
        model_name=model_name,
...
def _pdf_add_kv_table(story, rows, styles):
    ...
    if not data:
        return
    table = Table(data, colWidths=[50 * mm, 124 * mm], hAlign="LEFT", repeatRows=0)
...
def _build_program_pdf(
    ...
    if not interactions:
        ...
        for idx, interaction in enumerate(interactions, 1):
            ...
            if surveys:
                ...
                survey_table = Table(
                    survey_rows,
                    colWidths=[22*mm, 20*mm, 24*mm, 22*mm, 24*mm, 62*mm],
                    repeatRows=1,
                    hAlign="LEFT",
                )
    ...
    if not rapid_logs:
        ...
        rapid_table = Table(
            rapid_rows,
            colWidths=[30*mm, 30*mm, 42*mm, 72*mm],
            repeatRows=1,
            hAlign="LEFT",
        )
    ...
    if st.session_state.get("confirm_clean_memory", False):
        ...
        if st.button(
            ...
            if event_ids:
                ...
                # RapidStateLog has a foreign key to Event, so delete the
                # dependent rapid logs before deleting their parent events.
...
if page == "Experience Designer":
    ...
    # Keep the selector synchronized with the program currently being edited
    # while still allowing the user to switch back to a genuinely new program.
    #
    # IMPORTANT: Streamlit does not allow a widget-backed session-state key to
    # be assigned after that widget has been instantiated in the same script
    # run.  Changes requested by buttons below are therefore staged in a
    # separate pending key and applied here, BEFORE the selectbox is created.
    ...
    # Choosing New program from the selector starts a separate draft without
    # changing any saved database record.
    if (
        selected_saved_label == "New program"
        and st.session_state.get("experience_editor_event_id") is not None
    ):
        st.session_state["experience_editor_mode"] = "new"
        st.session_state["experience_editor_event_id"] = None
        st.session_state["active_event_id"] = None
    ...
    # Explicitly load the selected saved program into the existing designer
    # controls. Nothing is modified in the database by merely selecting it.
    if selected_saved_label != "New program" and saved_event_map:
        selected_event = saved_event_map[selected_saved_label]
        loaded_id = st.session_state.get("experience_editor_event_id")

        if loaded_id != selected_event.id:
            st.session_state["experience_editor_mode"] = "edit"
            st.session_state["experience_editor_event_id"] = selected_event.id
            st.session_state["active_event_id"] = selected_event.id
            st.session_state["active_interaction_id"] = None
            st.session_state["last_forward_model"] = None

            st.session_state["mem_event_name"] = selected_event.name or ""
            st.session_state["mem_objective"] = (
                selected_event.objective
                if selected_event.objective in [
                    "Curiosity",
                    "Scientific understanding",
                    "Awe and wonder",
            ...
            st.session_state["mem_audience"] = (
                selected_event.target_audience
                if selected_event.target_audience in [
                    "General public",
                    "Students",
                    "Families",
                    "Educators",
            ...
            st.session_state["mem_environment"] = selected_event.environment or ""
            st.session_state["mem_acoustic"] = selected_event.acoustic_environment or ""
            st.session_state["mem_sensory"] = selected_event.sensory_environment or ""
            st.session_state["mem_context"] = selected_event.context or ""
            st.session_state["mem_pacing"] = (
                selected_event.pacing or "Slow and contemplative"
            )
            st.session_state["mem_interaction_style"] = (
                selected_event.interaction_style or "Open observation"
            )
            st.session_state["mem_optional_choice"] = (
                selected_event.participant_autonomy or "High"
            )
...
"""
    ...
    if st.button(
        ...
        if not event_name.strip():
            ...
            try:
                ...
                _set_analysis_result("last_forward_model", event.id, model.model_dump())
    ...
    # `event` is created only after the Save/Create button is pressed.
    # On the initial render there is no local `event` variable, so use the
    # already-loaded editor event for display and show nothing in New mode.
    model = (
        _analysis_result_for_event("last_forward_model", editing_event.id)
        if editing_event is not None
        else None
    )
    if model:
            ...
            with st.expander(
                ...
                with ec2:
                    if st.button(
                        ...
                        st.session_state["experience_editor_mode"] = "edit"
                        st.session_state["experience_editor_event_id"] = saved_event.id
                        st.session_state["active_event_id"] = saved_event.id
                        st.session_state["active_interaction_id"] = None
                        st.session_state["last_forward_model"] = None

                        st.session_state["mem_event_name"] = saved_event.name or ""
                        st.session_state["mem_objective"] = (
                            saved_event.objective
                            if saved_event.objective in obj_opts
                            else "Curiosity"
                        )
                        st.session_state["mem_audience"] = (
                            saved_event.target_audience
                            if saved_event.target_audience in aud_opts
                            else "General public"
                        )
                        st.session_state["mem_environment"] = saved_event.environment or ""
                        st.session_state["mem_acoustic"] = saved_event.acoustic_environment or ""
                        st.session_state["mem_sensory"] = saved_event.sensory_environment or ""
                        st.session_state["mem_context"] = saved_event.context or ""
                        st.session_state["mem_pacing"] = saved_event.pacing or pac_opts[0]
                        st.session_state["mem_interaction_style"] = (
                            saved_event.interaction_style or style_opts[0]
                        )
                        st.session_state["mem_optional_choice"] = (
                            saved_event.participant_autonomy or aut_opts[0]
                        )
                        st.rerun()
            ...
            if client is None:
                ...
                try:
                    with st.spinner("Analyzing parameters and computing predicted outcomes..."):
                        ...
                        _set_analysis_result("last_prediction", event.id, pred.model_dump())
                except Exception as exc:
                    st.error(f"Prediction failed: {exc}")

        p = _analysis_result_for_event("last_prediction", event.id)
        if p:
...
# ============================================================
    ...
    render_html('<div class="small-note" style="margin-bottom:15px;">Explore hypothetical behavioral differences using HEXACO trait profiles and a user-defined resilience parameter. These are scenario simulations, not assessments or predictions of real individuals.</div>')
    ...
    with col_create:
        ...
        with st.form("create_personality", clear_on_submit=True):
            ...
            res_val = st.slider("Resilience Baseline Capacity", 0, 100, 50, help="A user-defined scenario parameter representing assumed recovery/adaptation capacity; it is not a clinical or validated measurement.")
            ...
            if client is None:
                ...
                try:
                    with st.spinner("Modeling behavioral impacts across diverse traits..."):
                        ...
                        _set_analysis_result("last_personality_prediction", event.id, impact_pred.model_dump())
                except Exception as exc:
                    st.error(f"Prediction failed: {exc}")
                    
        result = _analysis_result_for_event("last_personality_prediction", event.id)
        if result:
...
# ============================================================
    ...
    if not events:
        ...
        if not st.session_state.active_interaction_id:
            ...
            if st.button(
                ...
                if client is None:
                    ...
                    try:
                        ...
                        _set_analysis_result("last_recommendation", event.id, recommendation.model_dump())
            ...
            recommendation = _analysis_result_for_event("last_recommendation", event.id)
            if recommendation:
...
# ============================================================
    ...
    if not events:
        ...
        if metrics is None:
            ...
            if st.button(
                ...
                if client is None:
                    ...
                    try:
                        ...
                        _set_analysis_result("last_impact_interpretation", event.id, interpretation.model_dump())
            ...
            interpretation = _analysis_result_for_event("last_impact_interpretation", event.id)
            if interpretation:
...
# ============================================================
    ...
    if not events:
        ...
        if st.button(
            ...
            if not variable_change.strip():
                ...
                try:
                    ...
                    _set_analysis_result("last_counterfactual", event.id, cf.model_dump())
        ...
        cf = _analysis_result_for_event("last_counterfactual", event.id)
        if cf:
...
try:
    ...
    # Release any connection retained by this Streamlit script run.
    db.close()
