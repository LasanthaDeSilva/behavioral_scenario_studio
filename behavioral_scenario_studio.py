...
for _column, _sql_type in (
    ("ambient_noise_classification", "VARCHAR"),
    ("ambient_noise_db", "FLOAT"),
):
    try:
        with engine.connect() as conn:
            conn.execute(text(f"ALTER TABLE events ADD COLUMN {_column} {_sql_type}"))
            conn.commit()
    except Exception:
        pass
...
for _column, _sql_type in (
    ...
    try:
        with engine.connect() as conn:
            conn.execute(text(f"ALTER TABLE rapid_state_logs ADD COLUMN {_column} {_sql_type}"))
            conn.commit()
    except Exception:
        pass
...
for _column, _sql_type in (
    ...
    try:
        with engine.connect() as conn:
            conn.execute(text(f"ALTER TABLE interactions ADD COLUMN {_column} {_sql_type}"))
            conn.commit()
    except Exception:
        pass
...
# ============================================================
# 4A. LIVE BROWSER AMBIENT AUDIO STATE
# ============================================================

AMBIENT_QUIET = "Quiet & Controlled"
AMBIENT_MODERATE = "Moderate Noise"
AMBIENT_HIGH = "High Loudness / Busy Crowd"
AMBIENT_MAX_AGE_SECONDS = 12.0


def classify_ambient_noise(db_value: Optional[float]) -> str:
    """Classify an estimated A-weighted browser level using the requested thresholds."""
    try:
        value = float(db_value)
    except (TypeError, ValueError):
        return AMBIENT_MODERATE
...
def _query_param_float(name: str) -> Optional[float]:
    try:
...
def sync_live_ambient_state_from_browser() -> None:
    """Consume the latest browser publication without forcing a rerun every 3 seconds."""
    db_value = _query_param_float("ambient_db")
    ts_value = _query_param_float("ambient_ts")
    active_raw = str(st.query_params.get("ambient_active") or "").strip().lower()

    now_ms = datetime.now(timezone.utc).timestamp() * 1000.0
    fresh = (
        db_value is not None
        and ts_value is not None
        and 0.0 <= (now_ms - ts_value) <= AMBIENT_MAX_AGE_SECONDS * 1000.0
    )

    if fresh:
        db_value = round(max(20.0, min(100.0, db_value)), 1)
        st.session_state["live_ambient_db"] = db_value
        st.session_state["live_ambient_classification"] = classify_ambient_noise(db_value)
        st.session_state["live_ambient_tracking_active"] = active_raw == "1"
        st.session_state["live_ambient_last_update_ms"] = ts_value
    else:
        st.session_state["live_ambient_db"] = None
        st.session_state["live_ambient_classification"] = AMBIENT_MODERATE
        st.session_state["live_ambient_tracking_active"] = False
        st.session_state["live_ambient_last_update_ms"] = None


def get_live_ambient_state() -> Dict[str, Any]:
    value = st.session_state.get("live_ambient_db")
    try:
    ...
    if value is not None and not math.isfinite(value):
        value = None
    return {
        "db": round(value, 1) if value is not None else None,
        "classification": classify_ambient_noise(value),
        "active": bool(st.session_state.get("live_ambient_tracking_active", False)),
    }


def render_live_ambient_audio_engine() -> None:
    """Render the cross-platform browser microphone analyzer."""
    components.html(
        r"""
        <style>
            :root {
            ...
            * { box-sizing: border-box; }
            ...
            .title { color:var(--text); font-size:14px; font-weight:650; }
            .meter { color:var(--text); font-size:25px; font-weight:650; margin:9px 0 3px; }
            .status { color:var(--muted); font-size:12px; line-height:1.45; min-height:34px; }
            button {
                width:100%; border:1px solid rgba(91,140,255,.55); border-radius:10px;
                padding:11px 12px; color:white;
                background:linear-gradient(135deg,#274b9a,#5b8cff);
                font:inherit; font-weight:650; cursor:pointer; margin-top:10px; min-height:44px;
            }
            button:disabled { opacity:.65; cursor:default; }
            .tiny { color:#71717a; font-size:10px; line-height:1.35; margin-top:8px; }
            .active { color:var(--success); } .fallback { color:var(--warning); }
        </style>

        <div class="audio-card">
            <div class="title">🎙️ Live Ambient Audio</div>
            <div id="meter" class="meter">— dB</div>
            <div id="status" class="status">Microphone access is off until you authorize it.</div>
            <button id="authorize">🎙️ Authorize Live Audio Tracking</button>
            <div class="tiny">
                Relative browser microphone amplitude; not a traceable sound-pressure-level measurement.
            </div>
        </div>
        ...
        (() => {
            ...
            const MIN_DB = 20;
            const MAX_DB = 100;
            const CAL_TARGET_DB = 38;
            let stream = null, ctx = null, source = null, hp = null, peak = null, shelf = null;
            let analyser = null, samples = null, raf = 0, reportTimer = 0, running = false;
            let smoothedDb = null, calibrationOffset = null, calibrationSamples = [];
            ...
            function classify(db) {
                if (db < 48) return "Quiet & Controlled";
                if (db <= 68) return "Moderate Noise";
                return "High Loudness / Busy Crowd";
            }

            function publish(db, active) {
                const safeDb = Number.isFinite(db) ? Math.max(MIN_DB, Math.min(MAX_DB, db)) : 58;
                const classification = classify(safeDb);
                const now = Date.now();
                meter.textContent = `${safeDb.toFixed(1)} dB`;
                status.innerHTML = active
                    ? `<span class="active">● Tracking Active</span> — ${classification}`
                    : `<span class="fallback">● Tracking Inactive</span> — ${classification}`;

                const payload = {
                    type: "outreach_live_ambient_audio",
                    version: 1,
                    db: Number(safeDb.toFixed(1)),
                    classification,
                    active: !!active,
                    timestamp_ms: now
                };
                ...
                // components.html is one-way. Keep the latest payload in the parent
                // URL without navigation; Python consumes it on the next normal rerun.
                try {
                    const url = new URL(window.parent.location.href);
                    url.searchParams.set("ambient_db", String(payload.db));
                    url.searchParams.set("ambient_classification", classification);
                    url.searchParams.set("ambient_active", active ? "1" : "0");
                    url.searchParams.set("ambient_ts", String(now));
                    window.parent.history.replaceState(window.parent.history.state, "", url.href);
                } catch (e) {}

                try {
                    window.parent.localStorage.setItem(
                        "outreach_live_ambient_audio", JSON.stringify(payload)
                    );
                } catch (e) {}
            }
            ...
            function cleanup() {
                ...
                ctx = null; source = null; hp = null; peak = null; shelf = null;
                analyser = null; samples = null; running = false;
            }

            function frame() {
                if (!running || !analyser || !samples) return;
                ...
                const rawDb = 20 * Math.log10(rms + EPSILON);

                if (calibrationOffset === null) {
                    calibrationSamples.push(rawDb);
                    if (calibrationSamples.length >= 18) {
                        const sorted = [...calibrationSamples].sort((a,b) => a-b);
                        const median = sorted[Math.floor(sorted.length / 2)];
                        calibrationOffset = CAL_TARGET_DB - median;
                        calibrationSamples = [];
                    }
                }

                const interimOffset = calibrationOffset === null
                    ? CAL_TARGET_DB - (calibrationSamples.length
                        ? calibrationSamples.reduce((a,b) => a+b,0) / calibrationSamples.length
                        : rawDb)
                    : calibrationOffset;

                const candidate = Math.max(MIN_DB, Math.min(MAX_DB, rawDb + interimOffset));
                smoothedDb = smoothedDb === null
                    ? candidate
                    : (0.82 * smoothedDb + 0.18 * candidate);
                meter.textContent = `${smoothedDb.toFixed(1)} dB`;
            ...
            async function start() {
                ...
                try {
                    if (!navigator.mediaDevices?.getUserMedia) {
                        throw new Error("Microphone capture requires a secure HTTPS browser context.");
                    }
                    ...
                    // Practical A-weighting-inspired chain: suppress sub-60 Hz rumble,
                    // emphasize the human speech-sensitive region, and gently lift highs.
                    hp = ctx.createBiquadFilter();
                    hp.type = "highpass"; hp.frequency.value = 60; hp.Q.value = 0.707;

                    peak = ctx.createBiquadFilter();
                    peak.type = "peaking"; peak.frequency.value = 2500;
                    peak.Q.value = 0.85; peak.gain.value = 3.0;

                    shelf = ctx.createBiquadFilter();
                    shelf.type = "highshelf"; shelf.frequency.value = 4000; shelf.gain.value = 1.5;
                    ...
                    smoothedDb = null;
                    running = true;
                    status.innerHTML = '<span class="active">● Tracking Active</span> — calibrating current environment…';
                    button.textContent = "🎙️ Live Audio Tracking Active";
                    frame();

                    reportTimer = setInterval(() => {
                        if (running && smoothedDb !== null) publish(smoothedDb, true);
                    }, REPORT_MS);
                    ...
                    stream.getTracks().forEach(track => {
                        track.addEventListener("ended", () => {
                            if (running) {
                                ...
                                button.textContent = "🎙️ Authorize Live Audio Tracking";
                                status.textContent = "Microphone stream ended. Authorize again to resume.";
                            }
                        });
                    });
                } catch (error) {
                    ...
                    status.textContent = error?.message
                        ...
                        : "Microphone authorization was unavailable.";
                }
            }

            button.addEventListener("click", start);

            // Recover the most recent browser value immediately after a rerender.
            try {
                const saved = JSON.parse(
                    window.parent.localStorage.getItem("outreach_live_ambient_audio") || "null"
                );
                if (saved && Number.isFinite(Number(saved.db))) {
                    publish(Number(saved.db), saved.active === true);
                }
            } catch (e) {}
...
DEFAULT_STATE = {
    ...
    "live_ambient_db": None,
    "live_ambient_classification": AMBIENT_MODERATE,
    "live_ambient_tracking_active": False,
    "live_ambient_last_update_ms": None,
}
...
db = db_session()
sync_live_ambient_state_from_browser()
...
def create_event(
    ...
    event = Event(
        ...
        ambient_noise_classification=get_live_ambient_state()["classification"],
        ambient_noise_db=get_live_ambient_state()["db"],
    )
...
def update_event(
    ...
    ambient = get_live_ambient_state()
    event.ambient_noise_classification = ambient["classification"]
    event.ambient_noise_db = ambient["db"]
    try:
        db.commit()
    except Exception:
        db.rollback()
...
def create_interaction(
    ...
    interaction = Interaction(
        ...
        started_at=utc_now(),
        phase="Approach",
        ambient_noise_classification=ambient["classification"],
        ambient_noise_db=ambient["db"],
    )
...
render_live_ambient_audio_engine()
...
# ============================================================
    ...
    if not events:
        ...
        with qc1:
            ...
            live_ambient = get_live_ambient_state()
            st.session_state["q_noise"] = live_ambient["classification"]
            noise_col, meter_col = st.columns([2.25, 1.0])
            with noise_col:
            ...
            with meter_col:
                if live_ambient["db"] is not None and live_ambient["active"]:
                    meter_text = f"🎙️ Live Ambient Meter: {live_ambient['db']:.1f} dB — Tracking Active"
                else:
                    meter_text = "🎙️ Live Ambient Meter: — dB — Tracking Inactive"
                render_html(
                    f"""
                    <div class="metric-card" style="min-height:0;margin-top:4px;padding:14px;">
                        <div class="metric-label">Live ambient</div>
                        <div style="color:var(--text);font-size:.9rem;line-height:1.45;">
                            {clean_text(meter_text)}
                        </div>
                    </div>
                    """
                )
        ...
        questionnaire_summary = f"Baseline Stress: {baseline_stress_q}, Noise level: {noise_sensory_q}, Live ambient dB: {get_live_ambient_state()['db']}, Duration: {duration_q}, Interaction density: {interaction_density_q}"
...
Crowd details: {crowd_info}
Situational context: {situation_info}
Live ambient classification: {get_live_ambient_state()["classification"]}
Live ambient estimated dB: {get_live_ambient_state()["db"]}
    ...
    if not events:
            ...
            for rlog in recent_rapid_logs:
                ...
                if c_del.button("Delete", key=f"del_rlog_{rlog.id}", help="Delete this log"):
                    try:
                        ...
                        db.rollback()
                        st.error(f"Could not delete the rapid log: {exc}")
        ...
        if not st.session_state.active_interaction_id:
            ...
            if phase != interaction.phase:
                ...
                ambient = get_live_ambient_state()
                interaction.ambient_noise_classification = ambient["classification"]
                interaction.ambient_noise_db = ambient["db"]
                try:
                    db.commit()
                except Exception as exc:
                    db.rollback()
                    st.error(f"Could not save the interaction phase: {exc}")
            ...
            if preference != (interaction.stated_preference or ""):
                ...
                try:
                    ...
                    db.rollback()
                    st.error(f"Could not save the participant preference: {exc}")
...
""",
            ...
            if st.button(
                ...
                try:
                    ...
                    db.rollback()
                    st.error(f"Could not save the interaction end state: {exc}")
                    st.stop()
