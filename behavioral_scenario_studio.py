    ...
    if not events:
        ...
        if not interactions:
            ...
            survey_timing = st.selectbox(
                ...
                help=(
                    "BASELINE = before the outreach interaction. "
                    "IMMEDIATE = immediately after the outreach interaction. "
                    "DELAYED_24H and DELAYED_7D = later follow-up outcomes."
                ),
            )

            # ------------------------------------------------------------
            # Separate time-point status
            # ------------------------------------------------------------
            # Each participant interaction has two distinct primary
            # measurement occasions for immediate change:
            #   BEFORE  -> BASELINE
            #   AFTER   -> IMMEDIATE
            #
            # The two records are intentionally independent. The same
            # measurement instrument is used at each occasion so that a
            # change score can be calculated from comparable measurements.
            stored_surveys = (
                db.query(Survey)
            ...
            st.caption(
                "Record the participant at two separate time points using the same "
                "questions and scales: BEFORE = BASELINE and AFTER = IMMEDIATE. "
                "All three numeric measures and the descriptive state are stored "
                "separately for each time point. Do not infer a diagnosis or "
                "clinical/psychological condition from these records."
            )

            status_col1, status_col2 = st.columns(2)

            with status_col1:
                before_status = "Recorded" if latest_baseline else "Not recorded"
                st.markdown(f"**BEFORE · BASELINE** — {before_status}")
                if latest_baseline:
                    st.caption(
                        f"Curiosity {latest_baseline.curiosity:g} / 10"
                        if latest_baseline.curiosity is not None
                        else "Curiosity — / 10"
                    )
                    st.caption(
                        f"Understanding {latest_baseline.understanding:g} / 100"
                        if latest_baseline.understanding is not None
                        else "Understanding — / 100"
                    )
                    st.caption(
                        f"Confidence {latest_baseline.confidence:g} / 10"
                        if latest_baseline.confidence is not None
                        else "Confidence — / 10"
                    )
                    if latest_baseline.before_state:
                        st.caption(
                            f"State: {latest_baseline.before_state}"
                        )
                else:
                    st.caption(
                        "Record the participant's pre-interaction measurements "
                        "and state."
                    )

            with status_col2:
                after_status = "Recorded" if latest_immediate else "Not recorded"
                st.markdown(f"**AFTER · IMMEDIATE** — {after_status}")
                if latest_immediate:
                    st.caption(
                        f"Curiosity {latest_immediate.curiosity:g} / 10"
                        if latest_immediate.curiosity is not None
                        else "Curiosity — / 10"
                    )
                    st.caption(
                        f"Understanding {latest_immediate.understanding:g} / 100"
                        if latest_immediate.understanding is not None
                        else "Understanding — / 100"
                    )
                    st.caption(
                        f"Confidence {latest_immediate.confidence:g} / 10"
                        if latest_immediate.confidence is not None
                        else "Confidence — / 10"
                    )
                    if latest_immediate.after_state:
                        st.caption(
                            f"State: {latest_immediate.after_state}"
                        )
                else:
                    st.caption(
                        "Record the participant's immediate post-interaction "
            ...
            # ------------------------------------------------------------
            # BASELINE and IMMEDIATE deliberately use the same measurement
            # dimensions. This makes the two time points comparable while
            # keeping every value in its own Survey record.
            if survey_timing in ("BASELINE", "IMMEDIATE"):
                is_baseline = survey_timing == "BASELINE"
                latest_for_timing = (
                    latest_baseline if is_baseline else latest_immediate
                )
                ...
                if is_baseline:
                    render_html(
                        ...
                        <div class="premium-card" style="margin-top:16px;">
                            <div class="eyebrow">BEFORE INTERACTION · BASELINE</div>
                            <div style="color:var(--text); font-size:1.02rem;">
                                Record all measures independently for the participant
                                before the outreach interaction begins.
                            </div>
                        </div>
                        """
                    )
                    ...
                    render_html(
                        """
                        <div class="premium-card" style="margin-top:16px;">
                            <div class="eyebrow">AFTER INTERACTION · IMMEDIATE</div>
                            <div style="color:var(--text); font-size:1.02rem;">
                                Record all measures independently for the participant
                ...
                if latest_for_timing is not None:
                    ...
                    if latest_for_timing.understanding is not None:
                        default_understanding = int(
                            max(
                                ...
                                min(
                                    100,
                                    round(float(latest_for_timing.understanding)),
                                ),
                            )
                        )
                    if latest_for_timing.confidence is not None:
                        default_confidence = int(
                            max(
                                1,
                                min(10, round(float(latest_for_timing.confidence))),
                            )
                        )
                ...
                with st.form(
                    ...
                    st.caption(
                        "Use the same scales at BASELINE and IMMEDIATE. These are "
                        "participant-outcome measures for this interaction, not "
                    ...
                    curiosity = st.slider(
                        ...
                        help=(
                            "Participant-reported interest or desire to know more "
                            "about the topic. 1 = very low; 10 = very high."
                        ),
                    )
                    ...
                    understanding = st.slider(
                        ...
                        help=(
                            "Participant-reported or facilitator-administered "
                            "understanding measure on a 0–100 scale. Use the same "
                            "meaning and procedure at both time points."
                        ),
                    )
                    ...
                    confidence = st.slider(
                        ...
                        help=(
                            "Participant-reported confidence in asking or answering "
                            "questions about the topic. 1 = very low; 10 = very high."
                        ),
                    )

                    st.markdown("### Descriptive participant state")
                    ...
                    if is_baseline:
                        state_text = st.text_area(
                            ...
                            value=(
                                ...
                                if latest_for_timing
                                and latest_for_timing.before_state
                                else ""
                            ),
                            help=(
                                "Record only what the participant reports or what "
                                "is directly observable before the interaction. "
                                "Do not infer a diagnosis or hidden mental state."
                            ),
                            placeholder=(
                                "Example: Participant says they are unfamiliar "
                                "with the topic and asks what the demonstration is about."
                            ),
                        )
                    else:
                        state_text = st.text_area(
                            ...
                            value=(
                                ...
                                if latest_for_timing
                                and latest_for_timing.after_state
                                else ""
                            ),
                            help=(
                                "Record only what the participant reports or what "
                                "is directly observable immediately after the "
                                "interaction. Do not infer a diagnosis or hidden "
                                "mental state."
                            ),
                            placeholder=(
                                "Example: Participant says the main idea is clearer "
                                "and asks a follow-up question about the demonstration."
                            ),
                        )
                    ...
                    recall = st.text_area(
                        ...
                        help=(
                            "Record the participant's own response. For a meaningful "
                            "post-interaction recall measure, use the IMMEDIATE record."
                        ),
                    )
                    ...
                    follow_through = st.checkbox(
                        ...
                        help=(
                            "This is a delayed outcome and is recorded only at "
                            "DELAYED_24H or DELAYED_7D."
                        ),
                    )

                    submitted = st.form_submit_button(
                        (
                            "Save BEFORE / BASELINE record"
                            if is_baseline
                            else "Save AFTER / IMMEDIATE record"
                        ),
                        use_container_width=True,
                    )

                    if submitted:
                        cleaned_state = state_text.strip()
                        ...
                        survey_kwargs = dict(
                            ...
                            before_state=cleaned_state if is_baseline and cleaned_state else None,
                            after_state=cleaned_state if not is_baseline and cleaned_state else None,
                            follow_through=None,
                        )

                        db.add(Survey(**survey_kwargs))
                        db.commit()

                        st.success(
                            (
                                "BEFORE / BASELINE record saved separately for this participant."
                                if is_baseline
                                else "AFTER / IMMEDIATE record saved separately for this participant."
                            )
                        )
                        app_rerun()

            else:
                # Delayed outcomes keep the existing delayed measurement options
                # and controls. Before/After state fields are intentionally not
                # accepted at delayed time points because they belong to BASELINE
                # and IMMEDIATE respectively.
                render_html(
                    """
                    <div class="premium-card" style="margin-top:16px;">
                        <div class="eyebrow">DELAYED OUTCOME</div>
                        <div style="color:var(--text); font-size:1.02rem;">
                            Record later outcomes at the selected delayed time point.
                            Before/After state records belong only to BASELINE and
                            IMMEDIATE.
                        </div>
                    </div>
                    """
                ...
                with st.form(
                    ...
                    if submitted:
                        ...
                        db.commit()
                        st.success(
                            f"{survey_timing} outcome recorded. "
                            "Before/After state records remain separate at BASELINE and IMMEDIATE."
                        )
                        app_rerun()

            # Re-query after any saved record on the next render and present
            # both independent state records for verification.
            stored_surveys = (
                db.query(Survey)
                .filter(Survey.interaction_id == interaction.id)
                .order_by(Survey.recorded_at.desc(), Survey.id.desc())
            ...
            render_html(
                '<div class="section-heading">Recorded before / after participant states</div>'
            )
            ...
            with state_col1:
                ...
                if latest_baseline:
                    ...
                    st.write(
                        latest_baseline.before_state
                        or "No descriptive Before state was entered."
                    )
                else:
                    st.info("Not recorded yet.")
            ...
            with state_col2:
                ...
                if latest_immediate:
                    ...
                    st.write(
                        latest_immediate.after_state
                        or "No descriptive After state was entered."
                    )
                else:
                    st.info("Not recorded yet.")

            st.caption(
                "The BEFORE and AFTER records are independent measurements of the "
                "same participant interaction. Numerical change is calculated from "
                "the paired numeric measurements only. The descriptive state text "
                "provides qualitative context and is not converted into a numeric "
                "score or treated as proof of causation."
            )
