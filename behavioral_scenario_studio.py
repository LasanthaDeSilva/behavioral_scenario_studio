...
# - A persistent external database URL is preferred in deployed environments.
# - Streamlit secrets are checked first so the database configuration survives
#   application restarts/redeploys independently of the source working directory.
# - OUTREACH_DATABASE_URL remains the primary environment-variable name.
# - DATABASE_URL is also accepted as a conventional deployment secret/env name.
# - SQLite remains the local-development fallback.
# - The application never expires or deletes registered accounts automatically.
# - A persistent database with backups is required for true cross-restart /
#   multi-instance durability; a local SQLite file cannot provide that guarantee
#   on hosts whose filesystem is ephemeral.
#
# Keep the local SQLite fallback anchored to this source file. Streamlit
# deployments may use different working directories across restarts; a
# working-directory-relative SQLite URL can otherwise open a second empty DB.
_DEFAULT_DATABASE_PATH = (
    __import__("pathlib").Path(__file__).resolve().parent
    / "ninolades_outreach_lab.db"
)
_DEFAULT_DATABASE_URL = "sqlite:///" + str(_DEFAULT_DATABASE_PATH)


def _configured_database_url() -> str:
    """Resolve the database URL without silently changing persistence backends.

    Priority:
      1. Streamlit secret OUTREACH_DATABASE_URL
      2. Streamlit secret DATABASE_URL
      3. environment OUTREACH_DATABASE_URL
      4. environment DATABASE_URL
      5. anchored local SQLite fallback

    This keeps existing deployments working while allowing production
    deployments to supply one durable database for every Streamlit instance.
    """
    secret_values = []
    try:
        for key in ("OUTREACH_DATABASE_URL", "DATABASE_URL"):
            try:
                value = st.secrets.get(key, "")
            except Exception:
                value = ""
            if value:
                secret_values.append(str(value).strip())
    except Exception:
        pass
...
# Do not silently substitute an in-memory database. An in-memory database would
# erase all accounts and application data whenever the process restarts.
if DATABASE_URL.startswith("sqlite:///:memory:"):
    raise RuntimeError(
        "The configured database URL points to an in-memory SQLite database. "
        "Use a persistent SQLite file or a persistent server database instead."
    )
...
/* Swiss-style restraint: preserve the existing palette while improving
   hierarchy, rhythm, focus visibility, and responsive control behavior. */
.stButton > button,
...
# Explicit SQLAlchemy type instances used by the declarative schema.
# Keeping Column and its type objects under the same SQLAlchemy installation
# prevents type-identity mismatches that can surface as _set_parent_with_dispatch
# errors on some deployed environments.
SA_STRING = sa.String()
SA_DATETIME = sa.DateTime()
SA_TEXT = sa.Text()
SA_FLOAT = sa.Float()
SA_INTEGER = sa.Integer()
SA_BOOLEAN = sa.Boolean()

@st.cache_resource
def get_db_engine():
    """Create a durable, restart-safe SQLAlchemy engine.

    SQLite is configured for concurrent Streamlit access with WAL and a
    bounded busy timeout. Server databases use connection pre-ping and a
    modest pool so stale connections after idle periods are transparently
    replaced.
    """
    engine_kwargs = {
        "pool_pre_ping": True,
    ...
    if DATABASE_URL.startswith("sqlite"):
        ...
        # Keep server-database connections healthy across long idle periods
        # without imposing a large connection footprint.
...
class Event(Base):
    ...
    id = Column(SA_STRING, primary_key=True)
    session_token = Column(SA_STRING, nullable=False, default="global")
    name = Column(SA_STRING, nullable=False)
    date = Column(SA_DATETIME, nullable=False)
    objective = Column(SA_STRING, nullable=False)
    context = Column(SA_TEXT, nullable=True)
    environment = Column(SA_STRING, nullable=True)
    sensory_environment = Column(SA_STRING, nullable=True)
    acoustic_environment = Column(SA_STRING, nullable=True)
    target_audience = Column(SA_STRING, nullable=True)
    pacing = Column(SA_STRING, nullable=True)
    interaction_style = Column(SA_STRING, nullable=True)
    participant_autonomy = Column(SA_STRING, nullable=True)
...
class Interaction(Base):
    ...
    id = Column(SA_STRING, primary_key=True)
    event_id = Column(SA_STRING, ForeignKey("events.id"), nullable=False)
    participant_code = Column(SA_STRING, nullable=False)

    started_at = Column(SA_DATETIME, nullable=False)
    ended_at = Column(SA_DATETIME, nullable=True)
    ...
    stated_preference = Column(SA_TEXT, nullable=True)
...
class Observation(Base):
    ...
    timestamp = Column(SA_DATETIME, nullable=False)
    category = Column(SA_STRING, nullable=False)
    detail = Column(SA_TEXT, nullable=False)
...
class Survey(Base):
    ...
    timing = Column(SA_STRING, nullable=False)
    ...
    recorded_at = Column(SA_DATETIME, nullable=True)

    curiosity = Column(SA_FLOAT, nullable=True)
    understanding = Column(SA_FLOAT, nullable=True)
    confidence = Column(SA_FLOAT, nullable=True)

    recall_text = Column(SA_TEXT, nullable=True)
    ...
    before_state = Column(SA_TEXT, nullable=True)
    after_state = Column(SA_TEXT, nullable=True)

    # Optional descriptive follow-up state. This is deliberately separate from
    # the immediate "after_state" so delayed observations never overwrite the
    # immediate post-interaction record.
    delayed_state = Column(SA_TEXT, nullable=True)

    follow_through = Column(SA_BOOLEAN, nullable=True)
...
class RapidStateLog(Base):
    ...
    participant_code = Column(SA_STRING, nullable=False)
    timestamp = Column(SA_DATETIME, nullable=False)
    baseline_level = Column(SA_STRING, nullable=False)
    current_state = Column(SA_STRING, nullable=False)
...
class PersonalityProfile(Base):
    ...
    session_token = Column(SA_STRING, nullable=False, default="global")
    name = Column(SA_STRING, nullable=False)
    description = Column(SA_TEXT, nullable=True)
    
    # HEXACO + Resilience Metrics
    hexaco_h = Column(SA_INTEGER, default=50) # Honesty-Humility
    hexaco_e = Column(SA_INTEGER, default=50) # Emotionality
    hexaco_x = Column(SA_INTEGER, default=50) # Extraversion
    hexaco_a = Column(SA_INTEGER, default=50) # Agreeableness
    hexaco_c = Column(SA_INTEGER, default=50) # Conscientiousness
    hexaco_o = Column(SA_INTEGER, default=50) # Openness
    resilience_baseline = Column(SA_INTEGER, default=50) 


class UserAccount(Base):
    __tablename__ = "user_accounts"
    id = Column(SA_STRING, primary_key=True)
    username = Column(SA_STRING, nullable=False, unique=True, index=True)
    password_hash = Column(SA_STRING, nullable=False)
    created_at = Column(SA_DATETIME, nullable=False)
    last_login_at = Column(SA_DATETIME, nullable=True)


class AuthSession(Base):
    __tablename__ = "auth_sessions"
    id = Column(SA_STRING, primary_key=True)
    user_id = Column(SA_STRING, ForeignKey("user_accounts.id"), nullable=False, index=True)
    token_hash = Column(SA_STRING, nullable=False, unique=True, index=True)
    created_at = Column(SA_DATETIME, nullable=False)
    last_seen_at = Column(SA_DATETIME, nullable=False)
    revoked = Column(SA_BOOLEAN, nullable=False, default=False)


class AppState(Base):
    __tablename__ = "app_states"
    user_id = Column(SA_STRING, ForeignKey("user_accounts.id"), primary_key=True)
    state_json = Column(SA_TEXT, nullable=False, default="{}")
    updated_at = Column(SA_DATETIME, nullable=False)
...
def _database_column_exists(table_name: str, column_name: str) -> bool:
    """Check schema state through SQLAlchemy without hiding real DB failures."""
...
def _add_database_column_if_missing(table_name: str, column_name: str, sql_definition: str):
    """Add one known column exactly once; surface all unexpected DB errors."""
    if _database_column_exists(table_name, column_name):
        return
...
for _table, _column, _definition in (
    ("events", "session_token", "VARCHAR DEFAULT 'global'"),
    ("rapid_state_logs", "session_token", "VARCHAR DEFAULT 'global'"),
    ("personality_profiles", "session_token", "VARCHAR DEFAULT 'global'"),
    ("events", "pacing", "VARCHAR DEFAULT 'Slow and contemplative'"),
    ("events", "interaction_style", "VARCHAR DEFAULT 'Open observation'"),
    ("events", "participant_autonomy", "VARCHAR DEFAULT 'High'"),
    ("surveys", "before_state", "TEXT"),
    ("surveys", "after_state", "TEXT"),
    ("surveys", "delayed_state", "TEXT"),
    ("surveys", "recorded_at", "DATETIME"),
):
    _add_database_column_if_missing(_table, _column, _definition)
...
SCIENTIFIC_UNDERSTANDING_RUBRIC = (
    "0 = no demonstrated understanding of the target concept; "
    "25 = recognizes a basic term or idea but cannot explain the core relationship; "
    "50 = explains the central idea in broadly correct terms with important gaps; "
    "75 = explains the central idea and key relationships accurately with minor gaps; "
    "100 = explains the target concept and key relationships accurately and can apply or explain them in a relevant example."
)

# Registered accounts are durable records. There is deliberately no account
# age, inactivity, or login-session age limit in this application.
# AuthSession rows remain valid until explicitly revoked or removed by an
# administrator/database operator. This prevents silent logout after a period
# of inactivity while keeping explicit revocation possible.
...
if authenticated_user:
    # 1. User is valid: persist the session locally and immediately remove the
    # bearer token from the visible address bar/history entry. The existing
    # login mechanism remains intact, but the token is no longer left visible
    # in the URL after authentication.
    components.html(f"""
        <script>
        try {{
            window.parent.localStorage.setItem("outreach_auth_session", {json.dumps(raw_auth_token)});
            const url = new URL(window.parent.location);
            if (url.searchParams.has("auth_session") || url.searchParams.has("session_id")) {{
    ...
    if raw_auth_token:
        # 2. An invalid or revoked token was found in the URL. Clear it out so the user starts cleanly logged out.
...
# ============================================================
# 6B. GENERAL AI ANALYSIS LAYER
# ============================================================

class AIAnalysisModel(BaseModel):
    overview: str
    interconnections: List[str] = Field(min_length=3, max_length=6)
    important_findings: List[str] = Field(min_length=2, max_length=6)
    evidence_and_limits: str
    possible_next_steps: List[str] = Field(min_length=2, max_length=6)
    caution: str


def generate_ai_analysis(client, model_name, analysis_area: str, context: str):
    """Generate cautious cross-variable analysis without replacing deterministic analytics."""
    prompt = f"""
ANALYSIS AREA:
{analysis_area}
...
Perform a careful scientific analysis of the supplied context.

Requirements:
- Use only the supplied context. Do not invent missing facts.
- Consider meaningful interconnections among variables, environmental factors,
  observations, stated preferences, measurements, and design choices where present.
- Distinguish OBSERVED, STATED, INFERRED, and HYPOTHESIS content in wording.
- Treat model-generated estimates as hypotheses, not measurements.
- Do not diagnose, classify, or infer clinical conditions or protected/sensitive traits.
- Do not infer an internal mental state from behavior alone.
- Do not claim causation from associations or pre/post changes.
- Do not perform authoritative arithmetic; use supplied deterministic measurements as given.
- Give practical, low-pressure, reversible, autonomy-preserving next steps.
- If evidence is insufficient, say so and identify what additional observation or
  measurement would reduce uncertainty.
- "No intervention" is a legitimate option when evidence does not justify action.
- Do not optimize for manipulation, compliance, or persuasion at the expense of autonomy.
- Do not turn percentages or model estimates into facts about people.
- Be precise, conservative, logically reasoned, and useful to a science-outreach facilitator.
"""
    return run_gemini(
        client=client, model_name=model_name, prompt=prompt,
        schema=AIAnalysisModel, system_instruction=AI_SYSTEM, temperature=0.2,
    )


def _store_event_analysis(session_key: str, payload: Optional[dict], event_id: Optional[str] = None):
    """Keep existing workspace analysis keys while recording their source event."""
    st.session_state[session_key] = payload
    if event_id:
        st.session_state[f"{session_key}_event_id"] = event_id


def _event_analysis_payload(session_snapshot: dict, session_key: str, event_id: str):
    """Return a legacy workspace analysis only when it belongs to the selected event."""
    if session_snapshot.get(f"{session_key}_event_id") != event_id:
        return None
    return session_snapshot.get(session_key)


def render_ai_analysis_button(*, key: str, state_key: str, analysis_area: str, context: str,
                              button_help: str = "Ask Gemini to analyse the currently displayed information, relevant interconnections, uncertainty, and possible next steps."):
    """Render an additive AI Analyse control and its result."""
    if st.button("AI Analyse", key=key, use_container_width=True, help=button_help):
        if client is None:
            st.error("Gemini is unavailable.")
        elif not context.strip():
            st.warning("There is not enough information to analyse yet.")
        else:
            try:
                with st.spinner("AI is analysing the available evidence and interconnections..."):
                    analysis = generate_ai_analysis(client, selected_model, analysis_area, context)
                st.session_state[state_key] = analysis.model_dump()
                st.session_state[f"{state_key}_event_id"] = st.session_state.get("active_event_id")
            except Exception as exc:
    ...
    if result:
        ...
        <div class="premium-card" style="border-color:rgba(91,140,255,.30);">
            <div class="eyebrow">AI analysis · {clean_text(analysis_area)}</div>
            <div style="color:var(--text); line-height:1.75;">{clean_text(result.get("overview", ""))}</div>
        </div>
        """)
        render_html('<div class="section-heading">Relevant interconnections</div>')
        for item in result.get("interconnections", []):
            st.markdown(f"- {clean_text(item)}")
        render_html('<div class="section-heading">Important findings</div>')
        for item in result.get("important_findings", []):
            st.markdown(f"- {clean_text(item)}")
        render_html('<div class="section-heading">Evidence & limits</div>')
        st.write(result.get("evidence_and_limits", ""))
        render_html('<div class="section-heading">Possible next steps</div>')
        for item in result.get("possible_next_steps", []):
            st.markdown(f"- {clean_text(item)}")
        render_html('<div class="section-heading">Scientific caution</div>')
        st.info(result.get("caution", ""))
...
def _followup_anchor(interaction):
    """Return the best available timestamp for delayed-outcome latency.

    A completed interaction uses ended_at because the follow-up interval is
    conceptually anchored to the end of outreach. Legacy/open interactions
    fall back to started_at so their records remain analyzable.
    """
    return interaction.ended_at or interaction.started_at


def _latest_by_timing(df, timing):
    """Return the newest recorded row per participant/timing."""
    if df.empty:
        return df
...
def _mean_change_between(left_df, right_df, column):
    """Mean paired change right - left; missing values are excluded, not zero."""
    if left_df.empty or right_df.empty:
        return None, 0
    paired = left_df[[column]].join(
        right_df[[column]],
        lsuffix="_left",
        rsuffix="_right",
        how="inner",
    ...
    if paired.empty:
        return None, 0
    valid = paired[f"{column}_left"].notna() & paired[f"{column}_right"].notna()
    if not valid.any():
        return None, 0
    changes = (
        pd.to_numeric(paired.loc[valid, f"{column}_right"], errors="coerce")
        - pd.to_numeric(paired.loc[valid, f"{column}_left"], errors="coerce")
    ).dropna()
    if changes.empty:
        return None, 0
    return round(float(changes.mean()), 2), int(len(changes))


def _mean_and_sd(series):
    values = pd.to_numeric(series, errors="coerce").dropna()
    if values.empty:
        return None, None, 0
    mean = float(values.mean())
    sd = float(values.std(ddof=1)) if len(values) >= 2 else None
    return round(mean, 2), (round(sd, 2) if sd is not None else None), int(len(values))


def _delayed_timing_summary(interactions, df, timing):
    """Describe one delayed follow-up point with timing-quality safeguards.

    Coverage counts recorded latest responses. Latency statistics use only
    timestamps that are not earlier than the interaction anchor. A timestamp
    before the anchor is retained as a data-quality anomaly rather than being
    silently converted to zero hours.
    """
    target_hours = 24.0 if timing == "DELAYED_24H" else 24.0 * 7.0
    latest = _latest_by_timing(df, timing)

    empty = {
        "timing": timing,
        "label": "24-hour follow-up" if timing == "DELAYED_24H" else "7-day follow-up",
        "target_hours": target_hours,
        "responses": 0,
        "coverage_pct": None,
        "paired_n": {"curiosity": 0, "understanding": 0, "confidence": 0},
        "curiosity_change_from_immediate": None,
        "understanding_change_from_immediate": None,
        "confidence_change_from_immediate": None,
        "curiosity_change_from_baseline": None,
        "understanding_change_from_baseline": None,
        "confidence_change_from_baseline": None,
        "mean_response_hours": None,
        "median_response_hours": None,
        "mean_deviation_hours": None,
        "median_deviation_hours": None,
        "late_response_count": 0,
        "early_response_count": 0,
        "anomalous_timestamp_count": 0,
        "recall_response_rate": None,
        "follow_through_rate": None,
        "follow_through_valid_n": 0,
    }
    if latest.empty:
        return empty

    interaction_by_id = {i.id: i for i in interactions}
    latency_rows = []
    anomaly_count = 0
    for interaction_id, row in latest.iterrows():
        interaction = interaction_by_id.get(interaction_id)
        recorded_at = row.get("recorded_at")
        if interaction is None or pd.isna(recorded_at):
            continue

        anchor_ts = pd.Timestamp(_followup_anchor(interaction))
        anchor_ts = (anchor_ts.tz_localize("UTC") if anchor_ts.tzinfo is None else anchor_ts.tz_convert("UTC"))
        recorded_ts = pd.Timestamp(recorded_at)
        recorded_ts = (recorded_ts.tz_localize("UTC") if recorded_ts.tzinfo is None else recorded_ts.tz_convert("UTC"))
        raw_hours = (recorded_ts - anchor_ts).total_seconds() / 3600.0
        ...
        latency_rows.append({
            ...
            "deviation_hours": raw_hours - target_hours,
        })

    latency_df = pd.DataFrame(latency_rows)
    responses = int(len(latest))
    participant_count = len(interactions)
    coverage_pct = round(responses / participant_count * 100, 1) if participant_count else None

    mean_response_hours = median_response_hours = None
    mean_deviation_hours = median_deviation_hours = None
    late_count = early_count = 0
    if not latency_df.empty:
        mean_response_hours = round(float(latency_df["actual_hours"].mean()), 2)
        median_response_hours = round(float(latency_df["actual_hours"].median()), 2)
        mean_deviation_hours = round(float(latency_df["deviation_hours"].mean()), 2)
        median_deviation_hours = round(float(latency_df["deviation_hours"].median()), 2)
        late_count = int((latency_df["deviation_hours"] > 0).sum())
        early_count = int((latency_df["deviation_hours"] < 0).sum())

    recall_valid = latest["recall_text"].fillna("").astype(str).str.strip().ne("")
    recall_response_rate = round(float(recall_valid.mean() * 100), 1) if responses else None

    follow_valid = latest["follow_through"].notna()
    follow_through_rate = (
        round(float(latest.loc[follow_valid, "follow_through"].astype(bool).mean() * 100), 1)
        if follow_valid.any() else None
    )

    immediate = _latest_by_timing(df, "IMMEDIATE")
    baseline = _latest_by_timing(df, "BASELINE")
    c_i, n_ci = _mean_change_between(immediate, latest, "curiosity")
    u_i, n_ui = _mean_change_between(immediate, latest, "understanding")
    f_i, n_fi = _mean_change_between(immediate, latest, "confidence")
    c_b, _ = _mean_change_between(baseline, latest, "curiosity")
    u_b, _ = _mean_change_between(baseline, latest, "understanding")
    f_b, _ = _mean_change_between(baseline, latest, "confidence")

    return {
        **empty,
        "responses": responses,
        "coverage_pct": coverage_pct,
        "paired_n": {"curiosity": n_ci, "understanding": n_ui, "confidence": n_fi},
        "curiosity_change_from_immediate": c_i,
        "understanding_change_from_immediate": u_i,
        "confidence_change_from_immediate": f_i,
        "curiosity_change_from_baseline": c_b,
        "understanding_change_from_baseline": u_b,
        "confidence_change_from_baseline": f_b,
        "mean_response_hours": mean_response_hours,
        "median_response_hours": median_response_hours,
        "mean_deviation_hours": mean_deviation_hours,
        "median_deviation_hours": median_deviation_hours,
        "late_response_count": late_count,
        "early_response_count": early_count,
        "anomalous_timestamp_count": anomaly_count,
        "recall_response_rate": recall_response_rate,
        "follow_through_rate": follow_through_rate,
        "follow_through_valid_n": int(follow_valid.sum()),
    }


def calculate_event_impact(db, event_id):
    """Calculate deterministic descriptive outcome metrics.

    Immediate change is paired BASELINE -> IMMEDIATE within each interaction.
    Delayed change is separately paired IMMEDIATE -> DELAYED_24H and
    IMMEDIATE -> DELAYED_7D. Delayed response latency is measured from the
    interaction end (or start for unfinished/legacy interactions) to the
    recorded follow-up timestamp.

    The latest complete record at each timing point is used. Missing values
    are excluded from each calculation rather than treated as zero. All
    results describe the recorded sample and do not establish causation.
    """
    ...
    base_empty = {
        ...
        "curiosity_change_n": 0,
        "baseline_understanding": None,
        "post_understanding": None,
        "understanding_change": None,
        "understanding_change_n": 0,
        "baseline_confidence": None,
        "post_confidence": None,
        "confidence_change": None,
        "confidence_change_n": 0,
        "follow_through_rate": None,
        "follow_through_valid_n": 0,
        "follow_through_yes_n": 0,
        "recall_rate": None,
        "delayed_24h": _delayed_timing_summary(
            interactions, pd.DataFrame(), "DELAYED_24H"
        ),
        "delayed_7d": _delayed_timing_summary(
            interactions, pd.DataFrame(), "DELAYED_7D"
        ),
    }
    ...
    curiosity_change, curiosity_n = _mean_change_between(
        baseline, immediate, "curiosity"
    )
    understanding_change, understanding_n = _mean_change_between(
        baseline, immediate, "understanding"
    )
    confidence_change, confidence_n = _mean_change_between(
        baseline, immediate, "confidence"
    )

    delayed_24h = _delayed_timing_summary(
        interactions, df, "DELAYED_24H"
    )
    delayed_7d = _delayed_timing_summary(
        interactions, df, "DELAYED_7D"
    )

    delayed_all = df[df["timing"].isin(["DELAYED_24H", "DELAYED_7D"])]
    follow_through_rate = None
    follow_through_participants = 0
    follow_through_yes_participants = 0
    if not delayed_all.empty:
        # The UI label is participant-level: one participant is counted once
        # even when both 24-hour and 7-day follow-up records exist.
        interaction_to_participant = {i.id: i.participant_code for i in interactions}
        valid = delayed_all[delayed_all["follow_through"].notna()].copy()
        if not valid.empty:
            valid["participant_code"] = valid["interaction_id"].map(interaction_to_participant)
            valid = valid[valid["participant_code"].notna()]
            participant_any = (
                valid.groupby("participant_code", dropna=True)["follow_through"]
                .apply(lambda values: bool(values.astype(bool).any()))
            )
            if not participant_any.empty:
                follow_through_participants = int(len(participant_any))
                follow_through_yes_participants = int(participant_any.sum())
                follow_through_rate = round(
                    follow_through_yes_participants / follow_through_participants * 100, 1
                )
    ...
    recall_rate = (
        round(float(len(immediate_with_recall) / len(immediate) * 100), 1)
        if len(immediate) > 0 else None
    )
    ...
    return {
        ...
        "immediate_n": int(len(immediate)),
        "delayed_24h_n": delayed_24h["responses"],
        "delayed_7d_n": delayed_7d["responses"],
        ...
        "curiosity_change_n": curiosity_n,
        ...
        "understanding_change_n": understanding_n,
        "baseline_confidence": mean_or_none(baseline.get("confidence")),
        "post_confidence": mean_or_none(immediate.get("confidence")),
        "confidence_change": confidence_change,
        "confidence_change_n": confidence_n,
        "follow_through_rate": follow_through_rate,
        "follow_through_valid_n": follow_through_participants,
        "follow_through_yes_n": follow_through_yes_participants,
...
Based on these parameters, produce cautious scenario-model estimates. These are
hypotheses about a designed scenario, not measured participant states. Do not imply
that a percentage is a physiological measurement, validated psychometric score, or
probability. Provide bounded, internally consistent estimates for:
1. focus_pct (scenario-model index, 0-100%)
2. stress_reduction_pct (scenario-model index, 0-100%; not a physiological stress measurement)
3. cognitive_load_pct (scenario-model index, 0-100%; not a measured cognitive-load value)
4. attention_retention_pct (scenario-model index, 0-100%; not a measured retention value)
...
def _pdf_add_outcome_charts(story, metrics, styles):
    """Add scale-aware PDF charts without mixing unlike measurement scales."""
    if not metrics:
        return

    chart_specs = [
        ("Curiosity (1–10)", metrics.get("baseline_curiosity"), metrics.get("post_curiosity"), 10),
        ("Scientific understanding (0–100)", metrics.get("baseline_understanding"), metrics.get("post_understanding"), 100),
        ("Confidence (1–10)", metrics.get("baseline_confidence"), metrics.get("post_confidence"), 10),
    ]
    valid = [(label, base, post, ymax) for label, base, post, ymax in chart_specs
             if base is not None or post is not None]
    if not valid:
        return

    _pdf_add_heading(story, "Outcome trajectory charts", styles, level=2)
    story.append(_pdf_paragraph(
        "Baseline and immediate values are shown separately by scale. Missing values are left blank; "
        "no score is imputed. These are descriptive sample summaries, not causal estimates.",
        styles["ReportNote"],
    ))

    for label, base, post, ymax in valid:
        drawing = Drawing(175 * mm, 52 * mm)
        chart = VerticalBarChart()
        chart.x = 22 * mm
        chart.y = 12 * mm
        chart.width = 125 * mm
        chart.height = 30 * mm
        chart.data = [[float(base) if base is not None else 0, float(post) if post is not None else 0]]
        chart.categoryAxis.categoryNames = ["Baseline", "Immediate"]
        chart.valueAxis.valueMin = 0
        chart.valueAxis.valueMax = ymax
        chart.valueAxis.valueStep = 2 if ymax == 10 else 20
        chart.barWidth = 18 * mm
        chart.groupSpacing = 18 * mm
        ...
        chart.bars[0].fillColor = colors.HexColor("#20242C")
        drawing.add(chart)
        story.append(Paragraph(xml_escape(label), styles["FieldLabel"]))
        story.append(drawing)
        story.append(Spacer(1, 1 * mm))


def _pdf_add_followup_chart(story, metrics, styles):
    d24 = metrics.get("delayed_24h") or {}
    d7 = metrics.get("delayed_7d") or {}
    vals = [d24.get("coverage_pct"), d7.get("coverage_pct")]
    if not any(v is not None for v in vals):
        return
    _pdf_add_heading(story, "Delayed follow-up coverage", styles, level=2)
    ...
    chart.width = 125 * mm
    chart.height = 32 * mm
    chart.data = [[float(v) if v is not None else 0 for v in vals]]
    chart.categoryAxis.categoryNames = ["24-hour", "7-day"]
    chart.valueAxis.valueMin = 0
    chart.valueAxis.valueMax = 100
    chart.valueAxis.valueStep = 20
    chart.barWidth = 22 * mm
    chart.barLabels.visible = True
    chart.barLabels.fontSize = 7
    chart.barLabels.nudge = 8
    chart.valueAxis.labels.fontSize = 7
    ...
    story.append(drawing)
    story.append(_pdf_paragraph(
        "Coverage is the percentage of event interactions with a latest response at each delayed timing point. "
        "The event-level 'any delayed follow-through' metric counts each participant once across both delayed points.",
        styles["ReportNote"],
    ))
...
def _build_program_pdf(
    ...
    if metrics is None:
        ...
        _pdf_add_outcome_charts(story, metrics, styles)
        _pdf_add_followup_chart(story, metrics, styles)
    ...
    story.append(_pdf_paragraph(
        ...
        "the active Streamlit workspace and explicitly associated with the selected "
        "program. Generated AI content remains a hypothesis rather than a measurement. "
        "Analysis from another program is excluded rather than relabeled.",
        styles["ReportNote"],
    ))

    analysis_items = [
        ("Engagement architecture / forward model", _event_analysis_payload(session_snapshot, "last_forward_model", event.id)),
        ("Outcome prediction", _event_analysis_payload(session_snapshot, "last_prediction", event.id)),
        ("Personality predictor", _event_analysis_payload(session_snapshot, "last_personality_prediction", event.id)),
        ("Live copilot recommendation", _event_analysis_payload(session_snapshot, "last_recommendation", event.id)),
        ("Impact interpretation", _event_analysis_payload(session_snapshot, "last_impact_interpretation", event.id)),
        ("Counterfactual analysis", _event_analysis_payload(session_snapshot, "last_counterfactual", event.id)),
    ]

    # Event-scoped AI Analyse results already use event-specific session keys.
    # Include only the selected program's keys, never a stale result from another program.
    for scoped_key, title in [
        (f"ai_analysis_impact_observatory_{event.id}", "Impact Observatory AI analysis"),
        (f"ai_analysis_live_copilot_{event.id}", "Live Copilot AI analysis"),
        (f"ai_analysis_outcome_predictor_{event.id}", "Outcome Predictor AI analysis"),
        (f"ai_analysis_personality_predictor_{event.id}", "Personality Predictor AI analysis"),
    ]:
        payload = session_snapshot.get(scoped_key)
        if payload:
            analysis_items.append((title, payload))
    any_analysis = False
    ...
    prediction_inputs_event_id = session_snapshot.get("last_prediction_inputs_event_id")
    counterfactual_inputs_event_id = session_snapshot.get("last_counterfactual_inputs_event_id")
    for label, key in input_map:
        if key in {"pred_crowd", "pred_situation", "q_stress", "q_noise", "q_duration", "q_density"}:
            if prediction_inputs_event_id != event.id:
                continue
        elif key in {"cf_variable_change", "cf_design_description"}:
            if counterfactual_inputs_event_id != event.id:
                continue
...
"""
    ...
    render_ai_analysis_button(
        key="ai_analyse_experience_designer",
        state_key="ai_analysis_experience_designer",
        analysis_area="Experience design",
        context=f"""
Experience name: {event_name}
Primary objective: {objective}
Target audience: {target_audience}
Physical environment: {environment}
Acoustic / musical environment: {acoustic_environment}
Relevant environmental conditions: {sensory_environment}
...
""",
        ...
        if not event_name.strip():
            ...
            try:
                ...
                _store_event_analysis(
                    "last_forward_model", model.model_dump(), event.id
                )
    ...
    if st.session_state.last_forward_model and st.session_state.get("last_forward_model_event_id") == event.id:
...
# ============================================================
    ...
    if not events:
        ...
        render_ai_analysis_button(
            key=f"ai_analyse_outcome_predictor_{event.id}",
            state_key=f"ai_analysis_outcome_predictor_{event.id}",
            analysis_area="Outcome prediction inputs",
            context=f"""
            ...
            if client is None:
                ...
                try:
                    with st.spinner("Analyzing parameters and computing predicted outcomes..."):
                        ...
                        _store_event_analysis(
                            "last_prediction", pred.model_dump(), event.id
                        )
                        st.session_state["last_prediction_inputs_event_id"] = event.id
                except Exception as exc:
                    st.error(f"Prediction failed: {exc}")

        if st.session_state.get("last_prediction") and st.session_state.get("last_prediction_event_id") == event.id:
            p = st.session_state.last_prediction

            render_html('<div class="section-heading">Scenario-model estimates (not participant measurements)</div>')
            
            # Metric cards
            pc1, pc2, pc3, pc4 = st.columns(4)
            pc1.metric("Model-estimated focus index", f"{p.get('focus_pct', 75)}%")
            pc2.metric("Model-estimated stress-reduction index", f"{p.get('stress_reduction_pct', 60)}%")
            pc3.metric("Model-estimated cognitive-load index", f"{p.get('cognitive_load_pct', 45)}%")
            pc4.metric("Model-estimated attention-retention index", f"{p.get('attention_retention_pct', 80)}%")
            ...
            st.caption(
                "These percentages are model-estimated scenario indices. They are hypotheses "
                "generated from the supplied design inputs, not measured participant values, "
                "validated clinical/psychological scores, or probabilities."
            )
...
# ============================================================
    ...
    render_html('<div class="small-note" style="margin-bottom:15px;">Model how scenario-defined HEXACO and resilience inputs may interact with an outreach scenario. These are synthetic scenario profiles, not assessments or predictions of real individuals.</div>')
    ...
    if not events:
        ...
        for profile in saved_profiles:
            if profile.name in selected_profiles:
                selected_profile_context.append(
                    f"{profile.name}: H={profile.hexaco_h}, E={profile.hexaco_e}, "
                    f"X={profile.hexaco_x}, A={profile.hexaco_a}, C={profile.hexaco_c}, "
                    f"O={profile.hexaco_o}, Resilience={profile.resilience_baseline}; "
                    f"Description={profile.description or 'None'}"
                )

        render_ai_analysis_button(
            key=f"ai_analyse_personality_predictor_{event.id}",
            state_key=f"ai_analysis_personality_predictor_{event.id}",
            analysis_area="Personality scenario analysis",
            context=f"""
Experience: {event.name}
Objective: {event.objective}
Target audience: {event.target_audience}
Environment: {event.environment}
Selected profiles:
{chr(10).join(selected_profile_context) or "No profiles selected."}

Interpret HEXACO values as scenario-model inputs, not diagnoses or definitive
            ...
            if client is None:
                ...
                try:
                    with st.spinner("Modeling behavioral impacts across diverse traits..."):
                        ...
                        _store_event_analysis(
                            "last_personality_prediction", impact_pred.model_dump(), event.id
                        )
                except Exception as exc:
                    st.error(f"Prediction failed: {exc}")
                    
        if st.session_state.get("last_personality_prediction") and st.session_state.get("last_personality_prediction_event_id") == event.id:
...
# ============================================================
    ...
    if not events:
        ...
        if not st.session_state.active_interaction_id:
            ...
            live_analysis_observations = get_recent_observations(db, interaction.id)
            live_analysis_text = "\n".join(
                f"[{o.evidence_level}] {o.category}: {o.detail}"
                for o in live_analysis_observations
            )
            render_ai_analysis_button(
                key=f"ai_analyse_live_copilot_{interaction.id}",
                state_key=f"ai_analysis_live_copilot_{interaction.id}",
                analysis_area="Live interaction evidence",
                context=f"""
Experience: {event.name}
Objective: {event.objective}
Environment: {event.environment}
Acoustic environment: {event.acoustic_environment}
Participant stated preference: {interaction.stated_preference or "None recorded"}
Current phase: {interaction.phase}
Recent observations:
{live_analysis_text or "No observations recorded."}
""",
            )
            ...
            if st.button(
                ...
                if client is None:
                    ...
                    try:
                        ...
                        _store_event_analysis(
                            "last_recommendation", recommendation.model_dump(), event.id
                        )
            ...
            if st.session_state.last_recommendation and st.session_state.get("last_recommendation_event_id") == event.id:
...
# ============================================================
    ...
    if not events:
        ...
        if not logs:
            ...
            reaction_summary = df_logs.to_dict(orient="records")
            render_ai_analysis_button(
                key=f"ai_analyse_scientific_reactions_{event.id}",
                state_key=f"ai_analysis_scientific_reactions_{event.id}",
                analysis_area="Scientific reaction patterns",
                context=f"""
Experience: {event.name}
Objective: {event.objective}
Rapid reaction records (one row per logged observation):
{json.dumps(reaction_summary, default=str, ensure_ascii=False)}

These are facilitator-recorded categorical observations. Do not treat them as
direct measurements of internal cognitive or physiological states.
""",
            )
...
# ============================================================
    ...
    render_html("""
    <div class="premium-card">
        <div class="eyebrow">Observed impact · longitudinal view</div>
        <div style="color:var(--text); line-height:1.7;">
            The observatory separates immediate change from what remains,
            changes, or is reported later. All quantitative results are
            calculated deterministically from participant records. Delayed
            response timing is shown explicitly so a late follow-up is never
            mistaken for a true 24-hour or 7-day measurement.
        </div>
    </div>
    """)
    ...
    if not events:
        st.info("No experiences have been created yet.")
    else:
        event_map = {
            f"{e.name} ({e.date.strftime('%Y-%m-%d %H:%M:%S')})": e
            for e in events
        }
        selected_name = st.selectbox(
            "Experience",
        ...
        event = event_map[selected_name]
        metrics = calculate_event_impact(db, event.id)
        ...
        if metrics is None:
            ...
            overview[0].metric("Participants", metrics["participants"])
            overview[1].metric(
                "Baseline",
                metrics["baseline_n"],
                help="Participants with a recorded BASELINE outcome.",
            )
            overview[2].metric(
                "Immediate",
                metrics["immediate_n"],
                help="Participants with a recorded IMMEDIATE outcome.",
            )
            overview[3].metric(
                "24-hour follow-up",
                f'{metrics["delayed_24h"]["coverage_pct"]:.1f}%'
                if metrics["delayed_24h"]["coverage_pct"] is not None else "—",
                help="Recorded 24-hour follow-ups as a percentage of all participant interactions.",
            )
            overview[4].metric(
                "7-day follow-up",
                f'{metrics["delayed_7d"]["coverage_pct"]:.1f}%'
                if metrics["delayed_7d"]["coverage_pct"] is not None else "—",
                help="Recorded 7-day follow-ups as a percentage of all participant interactions.",
            )

            # ------------------------------------------------------------
            # Immediate change
            # ------------------------------------------------------------
            render_html('<div class="section-heading">Immediate outcome change</div>')
            immediate_rows = [
                {
                    "Measure": "Curiosity",
                    "Baseline": metrics["baseline_curiosity"],
                    "Immediate": metrics["post_curiosity"],
                    "Change": metrics["curiosity_change"],
                    "Paired n": metrics["curiosity_change_n"],
                    "Scale": "1–10",
                },
                {
                    "Measure": "Scientific understanding",
                    "Baseline": metrics["baseline_understanding"],
                    "Immediate": metrics["post_understanding"],
                    "Change": metrics["understanding_change"],
                    "Paired n": metrics["understanding_change_n"],
                    "Scale": "0–100",
                },
                {
                    "Measure": "Confidence",
                    "Baseline": metrics["baseline_confidence"],
                    "Immediate": metrics["post_confidence"],
                    "Change": metrics["confidence_change"],
                    "Paired n": metrics["confidence_change_n"],
                    "Scale": "1–10",
                },
            ]
            ...
            st.caption(
                "Change is calculated within the same participant interaction "
                "(IMMEDIATE − BASELINE). Missing values are excluded. Paired n "
                "is shown because the denominator can differ by measure."
            )

            # ------------------------------------------------------------
            # Delayed persistence / change
            # ------------------------------------------------------------
            render_html('<div class="section-heading">Delayed outcome trajectory</div>')
            st.caption(
                "Positive change below means the delayed value is higher than "
                "the participant's IMMEDIATE value. Negative change means it is lower. "
                "This is descriptive longitudinal evidence, not proof of causation."
            )

            for delayed_key in ("delayed_24h", "delayed_7d"):
                d = metrics[delayed_key]
                title = d["label"]
                with st.container():
                    render_html(f"""
                    <div class="premium-card">
                        <div class="eyebrow">{clean_text(title)}</div>
                        <div style="color:var(--text); line-height:1.7;">
                            {d["responses"]} recorded response(s) ·
                            {("—" if d["coverage_pct"] is None else f'{d["coverage_pct"]:.1f}%')} participant coverage.
                            Target interval: {d["target_hours"]:.0f} hours.
                        </div>
                    </div>
                    """)
                    ...
                    dc = st.columns(4)
                    dc[0].metric(
                        "Curiosity vs immediate",
                        "—" if d["curiosity_change_from_immediate"] is None
                        else f'{d["curiosity_change_from_immediate"]:+.2f}',
                        help=f'Paired n = {d["paired_n"]["curiosity"]}.',
                    )
                    dc[1].metric(
                        "Understanding vs immediate",
                        "—" if d["understanding_change_from_immediate"] is None
                        else f'{d["understanding_change_from_immediate"]:+.2f}',
                        help=f'Paired n = {d["paired_n"]["understanding"]}.',
                    )
                    dc[2].metric(
                        "Confidence vs immediate",
                        "—" if d["confidence_change_from_immediate"] is None
                        else f'{d["confidence_change_from_immediate"]:+.2f}',
                        help=f'Paired n = {d["paired_n"]["confidence"]}.',
                    )
                    dc[3].metric(
                        "Follow-through",
                        "—" if d["follow_through_rate"] is None
                        else f'{d["follow_through_rate"]:.1f}%',
                    )

                    timing_rows = [{
                        "Response timing": title,
                        "Median elapsed": (
                            "—" if d["median_response_hours"] is None
                            else f'{d["median_response_hours"]:.1f} h'
                        ),
                        "Median deviation from target": (
                            "—" if d["median_deviation_hours"] is None
                            else f'{d["median_deviation_hours"]:+.1f} h'
                        ),
                        "Mean elapsed": (
                            "—" if d["mean_response_hours"] is None
                            else f'{d["mean_response_hours"]:.1f} h'
                        ),
                        "Earlier than target": d["early_response_count"],
                        "Later than target": d["late_response_count"],
                        "Recall response": (
                            "—" if d["recall_response_rate"] is None
                            else f'{d["recall_response_rate"]:.1f}%'
                        ),
                    }]
                    st.dataframe(
                    ...
                    trajectory_rows = [
                        {
                            "Measure": "Curiosity",
                            "Immediate → delayed": d["curiosity_change_from_immediate"],
                            "Baseline → delayed": d["curiosity_change_from_baseline"],
                            "Paired n (immediate → delayed)": d["paired_n"]["curiosity"],
                        },
                        {
                            "Measure": "Scientific understanding",
                            "Immediate → delayed": d["understanding_change_from_immediate"],
                            "Baseline → delayed": d["understanding_change_from_baseline"],
                            "Paired n (immediate → delayed)": d["paired_n"]["understanding"],
                        },
                        {
                            "Measure": "Confidence",
                            "Immediate → delayed": d["confidence_change_from_immediate"],
                            "Baseline → delayed": d["confidence_change_from_baseline"],
                            "Paired n (immediate → delayed)": d["paired_n"]["confidence"],
                        },
                    ]
                    st.dataframe(
            ...
            signal_data = {
                "Curiosity": metrics["curiosity_change"],
                "Understanding": metrics["understanding_change"],
                "Confidence": metrics["confidence_change"],
            }
            signal_df = pd.DataFrame([
                {"Signal": name, "Immediate change": value}
            ...
            if not signal_df.empty:
                ...
                st.info(
                    "Paired BASELINE → IMMEDIATE measurements are needed to calculate "
                    "immediate change."
                )

            render_ai_analysis_button(
                key=f"ai_analyse_impact_observatory_{event.id}",
                state_key=f"ai_analysis_impact_observatory_{event.id}",
                analysis_area="Impact observatory",
                context=f"""
Experience: {event.name}
Objective: {event.objective}
Deterministic event-level metrics:
{json.dumps(metrics, default=str, ensure_ascii=False)}

These metrics are descriptive evidence from recorded participant data. Consider
incomplete follow-up, response timing, measurement limits, selection effects,
novelty, facilitator effects, and other alternative explanations. Do not claim causation.
""",
            )

            render_html('<div class="section-heading">Recall and follow-through</div>')
            ri1, ri2, ri3 = st.columns(3)
            ri1.metric(
                "Immediate recall response",
                "—" if metrics["recall_rate"] is None else f'{metrics["recall_rate"]:.1f}%'
            )
            ri2.metric(
                "Any delayed follow-through",
                "—" if metrics["follow_through_rate"] is None
                else f'{metrics["follow_through_rate"]:.1f}%'
            )
            ri3.metric("Participants with any survey", metrics["surveyed"])

            st.caption(
                "A recall response rate indicates that a response was recorded; "
                "it does not by itself establish recall accuracy. Follow-through "
                "is based on the participant's recorded voluntary report."
            )

            # ------------------------------------------------------------
            # Data-quality / interpretation guardrails
            # ------------------------------------------------------------
            render_html('<div class="section-heading">Measurement quality</div>')
            quality_rows = []
            for timing, key in [
                ("BASELINE", "baseline_n"),
                ("IMMEDIATE", "immediate_n"),
                ("DELAYED_24H", "delayed_24h_n"),
                ("DELAYED_7D", "delayed_7d_n"),
            ]:
                quality_rows.append({
                    "Measurement point": timing,
                    "Recorded participants": metrics[key],
                    "Coverage": (
                        f'{metrics[key] / metrics["participants"] * 100:.1f}%'
                        if metrics["participants"] else "—"
                    ),
                })
            st.dataframe(
            ...
            st.caption(
                "Delayed timing is never silently treated as exact. The observatory "
                "reports the actual elapsed interval and deviation from the intended "
                "24-hour or 7-day target for each latest participant record. "
                "Records timestamped before the interaction anchor are retained but "
                "excluded from latency statistics until corrected."
            )

            # ------------------------------------------------------------
            # AI interpretation remains interpretive, not computational
            # ------------------------------------------------------------
            render_html('<div class="section-heading">AI interpretation</div>')
            st.caption(
                "Gemini interprets the deterministic measurements below; it does not "
                "calculate them. Interpretation should consider delayed-response timing "
                "and incomplete follow-up coverage."
            )
            ...
            if st.button(
                ...
                type="primary",
                use_container_width=True,
                key=f"interpret_impact_{event.id}",
            ):
                if client is None:
                    st.error("Gemini is unavailable.")
                else:
                    try:
                        with st.spinner("Interpreting outcome patterns..."):
                            interpretation = generate_impact_interpretation(
                                client,
                                selected_model,
                                metrics,
                            )
                        _store_event_analysis(
                            "last_impact_interpretation", interpretation.model_dump(), event.id
                        )
                    except Exception as exc:
                        st.error(f"Impact interpretation failed: {exc}")

            if st.session_state.last_impact_interpretation and st.session_state.get("last_impact_interpretation_event_id") == event.id:
                interpretation = st.session_state.last_impact_interpretation
                ...
                with c1:
                    ...
                    st.write(interpretation["strongest_signal"])
                with c2:
                    render_html('<div class="section-heading">Weakest signal</div>')
                    st.write(interpretation["weakest_signal"])
                ...
                st.info(interpretation["recommended_next_test"])
...
# ============================================================
    ...
    if not events:
        ...
        render_ai_analysis_button(
            key=f"ai_analyse_counterfactual_lab_{event.id}",
            state_key=f"ai_analysis_counterfactual_lab_{event.id}",
            analysis_area="Counterfactual design reasoning",
            context=f"""
Experience: {event.name}
Objective: {event.objective}
Environment: {event.environment}
Current design:
{design_description}
Proposed variable change:
{variable_change}

Treat this as theoretical counterfactual reasoning only. Do not present the
comparison as experimental evidence or a causal estimate.
""",
        )
        ...
        if st.button(
            ...
            if not variable_change.strip():
                ...
                try:
                    ...
                    _store_event_analysis(
                        "last_counterfactual", cf.model_dump(), event.id
                    )
                    st.session_state["last_counterfactual_inputs_event_id"] = event.id
        ...
        if st.session_state.last_counterfactual and st.session_state.get("last_counterfactual_event_id") == event.id:
...
# ============================================================
    ...
    if not report_events:
        ...
        report_session_keys = [
            "last_forward_model", "last_forward_model_event_id",
            "last_prediction", "last_prediction_event_id", "last_prediction_inputs_event_id",
            "last_personality_prediction", "last_personality_prediction_event_id",
            "last_recommendation", "last_recommendation_event_id",
            "last_impact_interpretation", "last_impact_interpretation_event_id",
            "last_counterfactual", "last_counterfactual_event_id", "last_counterfactual_inputs_event_id",
            "pred_crowd", "pred_situation", "q_stress", "q_noise",
            "q_duration", "q_density", "cf_variable_change", "cf_design_description",
        ]
        report_session_keys.extend([
            f"ai_analysis_impact_observatory_{report_event.id}",
            f"ai_analysis_live_copilot_{report_event.id}",
            f"ai_analysis_outcome_predictor_{report_event.id}",
            f"ai_analysis_personality_predictor_{report_event.id}",
        ])
...
if page in ("Live Copilot", "Impact Observatory"):
    st.markdown("---")
    ...
    with st.expander(
        ...
        st.markdown(
            """
            This module is optional and secondary to the outreach intelligence system.
            It records the same three outcome measures across four clearly separated
            measurement points so immediate change and delayed trajectory can be
            compared without overwriting earlier records.
            """
        )
        ...
        if not events:
            ...
            if not interactions:
                ...
                timing_labels = {
                    ...
                    "IMMEDIATE": "After · Immediate",
                    "DELAYED_24H": "Follow-up · 24 hours",
                    "DELAYED_7D": "Follow-up · 7 days",
                }
                survey_timing = st.selectbox(
                    ...
                    help=(
                        "Use the same three numeric measures at every point. "
                        "BASELINE is before outreach; IMMEDIATE is directly after; "
                        "DELAYED_24H and DELAYED_7D are later follow-ups."
                    ),
                )
                ...
                def latest_survey_for(timing):
                    return next(
                        (s for s in stored_surveys if s.timing == timing),
                        None,
                    )

                latest_by_point = {
                    timing: latest_survey_for(timing)
                    for timing in timing_opts
                }

                # ------------------------------------------------------------
                # Clean four-point progress strip
                # ------------------------------------------------------------
                render_html('<div class="section-heading">Outcome timeline</div>')
                point_cols = st.columns(4)
                for col, timing in zip(point_cols, timing_opts):
                    record = latest_by_point[timing]
                    with col:
                        status = "Recorded" if record else "Not recorded"
                        if timing == "DELAYED_24H":
                            anchor = _followup_anchor(interaction)
                            target = pd.Timestamp(anchor)
                            if target.tzinfo is None:
                                target = target.tz_localize("UTC")
                            else:
                                target = target.tz_convert("UTC")
                            timing_note = "Target +24 h"
                        elif timing == "DELAYED_7D":
                            anchor = _followup_anchor(interaction)
                            target = pd.Timestamp(anchor)
                            if target.tzinfo is None:
                                target = target.tz_localize("UTC")
                                ...
                                target = target.tz_convert("UTC")
                            timing_note = "Target +7 d"
                        else:
                            target = None
                            timing_note = (
                                "Before outreach" if timing == "BASELINE"
                                else "Immediately after"
                            )

                        render_html(f"""
                        <div class="metric-card" style="min-height:145px;">
                            <div class="metric-label">{clean_text(timing_labels[timing])}</div>
                            <div class="metric-value" style="font-size:1.15rem;">
                                {status}
                            </div>
                            <div class="metric-sub">{timing_note}</div>
                        </div>
                        """)

                        if record:
                            st.caption(
                                f"Curiosity {record.curiosity:g}/10 · "
                                f"Understanding {record.understanding:g}/100 · "
                                f"Confidence {record.confidence:g}/10"
                            )

                # ------------------------------------------------------------
                # Measurement form
                # ------------------------------------------------------------
                is_baseline = survey_timing == "BASELINE"
                is_immediate = survey_timing == "IMMEDIATE"
                is_delayed = survey_timing in ("DELAYED_24H", "DELAYED_7D")
                latest_for_timing = latest_by_point[survey_timing]

                if is_baseline:
                    eyebrow = "BEFORE INTERACTION · BASELINE"
                    description = (
                        "Record the participant before outreach begins. Use only "
                        "participant-stated or directly observable information."
                    )
                elif is_immediate:
                    eyebrow = "AFTER INTERACTION · IMMEDIATE"
                    description = (
                        "Record the participant immediately after the interaction "
                        "using the same measures used at BASELINE."
                    )
                elif survey_timing == "DELAYED_24H":
                    eyebrow = "FOLLOW-UP · 24 HOURS"
                    description = (
                        "Record the participant's later outcome. The app will compare "
                        "the recorded time with the actual 24-hour target."
                    )
                else:
                    eyebrow = "FOLLOW-UP · 7 DAYS"
                    description = (
                        "Record the participant's later outcome. The app will compare "
                        "the recorded time with the actual 7-day target."
                    )

                render_html(f"""
                <div class="premium-card" style="margin-top:16px;">
                    <div class="eyebrow">{eyebrow}</div>
                    <div style="color:var(--text); font-size:1.02rem;">
                        {description}
                    </div>
                </div>
                """)
                ...
                if is_delayed:
                    anchor = _followup_anchor(interaction)
                    target_hours = 24.0 if survey_timing == "DELAYED_24H" else 168.0
                    now = utc_now()
                    ...
                    elapsed_now = max(
                        0.0, (pd.Timestamp(now) - anchor_ts).total_seconds() / 3600.0
                    )
                    st.info(
                        f"Follow-up timing is anchored to the interaction "
                        f"{'end' if interaction.ended_at else 'start'}. "
                        f"Target: +{target_hours:.0f} hours. "
                        f"Current elapsed time: {elapsed_now:.1f} hours. "
                        "The exact save timestamp will be used for the final latency calculation."
                    )

                # Defaults are intentionally populated from the latest record at
                # this point, but saving always creates a new immutable observation.
                default_curiosity = 5
                default_understanding = 50
                default_confidence = 5
                default_recall = ""
                default_delayed_state = ""

                if latest_for_timing is not None:
                    if latest_for_timing.curiosity is not None:
                        default_curiosity = int(
                            max(1, min(10, round(float(latest_for_timing.curiosity))))
                        )
                    if latest_for_timing.understanding is not None:
                        default_understanding = int(
                            max(0, min(100, round(float(latest_for_timing.understanding))))
                        )
                    if latest_for_timing.confidence is not None:
                        default_confidence = int(
                            max(1, min(10, round(float(latest_for_timing.confidence))))
                        )
                    default_recall = latest_for_timing.recall_text or ""
                    default_delayed_state = getattr(
                        latest_for_timing, "delayed_state", None
                    ) or ""

                with st.form(
                    f"outcome_capture_{survey_timing.lower()}",
                    clear_on_submit=True,
                ):
                    st.markdown("### Numeric measures")
                    st.caption(
                        "Keep the wording and meaning of each scale consistent across "
                        "all time points. These are lightweight outcome measures, not "
                        "clinical or diagnostic scores."
                    )
                    ...
                    with c1:
                        curiosity = st.slider(
                            "Curiosity",
                            1, 10, default_curiosity,
                            key=f"curiosity_{survey_timing.lower()}",
                            help="Participant-reported interest or desire to know more. 1 = very low; 10 = very high.",
                        )
                    with c2:
                        understanding = st.slider(
                            "Scientific understanding",
                            0, 100, default_understanding,
                            key=f"understanding_{survey_timing.lower()}",
                            help=(
                                "Use the same participant-reported or facilitator-administered measure each time. "
                                + SCIENTIFIC_UNDERSTANDING_RUBRIC
                            ),
                        )
                    with c3:
                        confidence = st.slider(
                            ...
                            1, 10, default_confidence,
                            key=f"confidence_{survey_timing.lower()}",
                            help="Confidence about asking or answering questions. 1 = very low; 10 = very high.",
                        )
                    ...
                    recall = st.text_area(
                        ...
                        key=f"recall_{survey_timing.lower()}",
                        help=(
                            "Record the participant's own response. A response being "
                            "present does not mean it is accurate."
                        ),
                    )
                    ...
                    if is_baseline:
                        state_text = st.text_area(
                            ...
                            height=100,
                            value=(
                                latest_for_timing.before_state
                                if latest_for_timing and latest_for_timing.before_state
                                else ""
                            ),
                            key="before_state_baseline",
                            help=(
                                "Record only what the participant reports or what is "
                                "directly observable before outreach."
                            ),
                            placeholder=(
                                "Example: Participant says they are unfamiliar with "
                                "the topic and asks what the demonstration is about."
                            ),
                        )
                    elif is_immediate:
                        state_text = st.text_area(
                            ...
                            height=100,
                            value=(
                                latest_for_timing.after_state
                                if latest_for_timing and latest_for_timing.after_state
                                else ""
                            ),
                            key="after_state_immediate",
                            help=(
                                "Record only what the participant reports or what is "
                                "directly observable immediately afterward."
                            ),
                            placeholder=(
                                "Example: Participant says the main idea is clearer "
                                "and asks a follow-up question."
                            ),
                        )
                    else:
                        state_text = st.text_area(
                            "Delayed participant state — optional",
                            height=100,
                            value=default_delayed_state,
                            key=f"delayed_state_{survey_timing.lower()}",
                            help=(
                                "Record only participant-stated or directly observed "
                                "information at this follow-up. This remains qualitative "
                                "context and is not converted into a score."
                            ),
                            placeholder=(
                                "Example: Participant reports that they kept thinking "
                                "about the demonstration and independently looked up "
                                "a related concept."
                            ),
                        )
                    ...
                    follow_through = st.checkbox(
                        ...
                        value=(
                            bool(latest_for_timing.follow_through)
                            if latest_for_timing and latest_for_timing.follow_through is not None
                            else False
                        ),
                        key=f"follow_through_{survey_timing.lower()}",
                        disabled=not is_delayed,
                        help=(
                            "A delayed voluntary follow-through outcome. It is only "
                            "recorded at 24-hour or 7-day follow-up."
                        ),
                    )
                    ...
                    submitted = st.form_submit_button(
                        (
                            ...
                            if is_baseline
                            else "Save AFTER / IMMEDIATE"
                            if is_immediate
                            else "Save 24-hour follow-up"
                            if survey_timing == "DELAYED_24H"
                            else "Save 7-day follow-up"
                        ),
                        type="primary",
                        use_container_width=True,
                    )
                    ...
                    if submitted:
                        cleaned_state = (state_text or "").strip()
                        survey = Survey(
                            id=str(uuid.uuid4()),
                            ...
                            before_state=(
                                cleaned_state
                                if is_baseline and cleaned_state else None
                            ),
                            after_state=(
                                cleaned_state
                                if is_immediate and cleaned_state else None
                            ),
                            delayed_state=(
                                cleaned_state
                                if is_delayed and cleaned_state else None
                            ),
                            follow_through=(
                                bool(follow_through) if is_delayed else None
                            ),
                        )
                        db.add(survey)
                        db.commit()
                        ...
                        st.success(
                            f"{timing_labels[survey_timing]} record saved. "
                            "The timestamp is stored automatically for delayed-response analysis."
                        )
                        app_rerun()

                # ------------------------------------------------------------
                # Participant-level change preview
                # ------------------------------------------------------------
                latest_baseline = latest_by_point["BASELINE"]
                latest_immediate = latest_by_point["IMMEDIATE"]
                latest_24h = latest_by_point["DELAYED_24H"]
                latest_7d = latest_by_point["DELAYED_7D"]

                render_html('<div class="section-heading">Participant trajectory</div>')
                trajectory = []
                for label, record in [
                    ("Baseline", latest_baseline),
                    ("Immediate", latest_immediate),
                    ("24-hour follow-up", latest_24h),
                    ("7-day follow-up", latest_7d),
                ]:
                    trajectory.append({
                        "Point": label,
                        "Curiosity / 10": None if record is None else record.curiosity,
                        "Understanding / 100": None if record is None else record.understanding,
                        "Confidence / 10": None if record is None else record.confidence,
                        "Recorded": "No" if record is None else (
                            record.recorded_at.strftime("%Y-%m-%d %H:%M UTC")
                            if record.recorded_at else "Yes"
                        ),
                    })
                st.dataframe(
                ...
                if latest_24h or latest_7d:
                    ...
                    for timing, record, target_hours in [
                        ("24-hour follow-up", latest_24h, 24.0),
                        ("7-day follow-up", latest_7d, 168.0),
                    ]:
                        if record and record.recorded_at:
                            recorded_ts = pd.Timestamp(record.recorded_at)
                            if recorded_ts.tzinfo is None:
                                recorded_ts = recorded_ts.tz_localize("UTC")
                            else:
                                recorded_ts = recorded_ts.tz_convert("UTC")
                            actual_hours = max(
                                0.0,
                                (recorded_ts - anchor_ts).total_seconds() / 3600.0,
                            )
                            latency_rows.append({
                                "Follow-up": timing,
                                "Target": f"{target_hours:.0f} h",
                                "Actual elapsed": f"{actual_hours:.1f} h",
                                "Deviation": f"{actual_hours - target_hours:+.1f} h",
                                "Timing": (
                                    "Later than target"
                                    if actual_hours > target_hours
                                    else "Earlier than target"
                                    if actual_hours < target_hours
                                    else "At target"
                                ),
                            })
                ...
                st.caption(
                    "All records remain separate. The observatory uses the latest saved "
                    "record for each measurement point when summarizing this participant, "
                    "while preserving every saved record in the database."
                )
