...
for _column in ("before_state", "after_state"):
    try:
        with engine.connect() as conn:
            conn.execute(
                text(
                    f"ALTER TABLE surveys ADD COLUMN {_column} TEXT"
                )
            )
            conn.commit()
    except Exception:
    ...
    if not events:
        ...
        if not interactions:
            ...
            with st.form(
                ...
                st.caption(
                    "Optional descriptive record. Use participant-reported wording "
                    "or directly observed, concrete descriptions only. Do not "
                    "diagnose, label, or infer a clinical or psychological condition."
                )
                ...
                before_state = st.text_area(
                    ...
                    height=80,
                    placeholder=(
                        "Describe the participant's state before the outreach "
                        "interaction (reported or directly observed)."
                    ),
                )
                ...
                after_state = st.text_area(
                    ...
                    height=80,
                    placeholder=(
                        "Describe the participant's state after the outreach "
                        "interaction (reported or directly observed)."
                    ),
                )
