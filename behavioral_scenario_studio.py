...
try:
    from streamlit.components.v2 import component as streamlit_component_v2
except Exception:
    streamlit_component_v2 = None
...
def classify_ambient_noise(amplitude: Optional[float]) -> str:
    """Classify normalized browser RMS amplitude using the fixed application matrix."""
    try:
    ...
    if value <= 0.45:
        return "Moderate Noise"
    return "High Loudness / Busy Crowd"


AMBIENT_AUDIO_HTML = r"""
<div class="ambient-audio-shell">
  <div class="ambient-audio-header">
    <div>
      <div class="ambient-eyebrow">LIVE AMBIENT AUDIO</div>
      <div class="ambient-title">Browser microphone tracking</div>
    </div>
    <span id="ambient-status" class="ambient-status">Not authorized</span>
  </div>
  <button id="ambient-authorize" class="ambient-button" type="button">
    🎙️ Authorize Live Audio Tracking
  </button>
  <div class="ambient-readout">
    <span id="ambient-level">Waiting for authorization</span>
    <span id="ambient-classification">Moderate Noise</span>
  </div>
  <div class="ambient-note">
    Audio is analyzed locally in this browser. Only a normalized amplitude snapshot and its categorical classification are sent to the Streamlit session.
  </div>
</div>
"""
...
.ambient-audio-shell {
  ...
  border-radius: 14px;
  background: linear-gradient(145deg, rgba(22,22,25,.98), rgba(11,11,13,.98));
  color: #f5f5f7;
  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif;
}
.ambient-audio-header { display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:10px; }
.ambient-eyebrow { color:#8aa9ff; font-size:10px; font-weight:700; letter-spacing:.12em; text-transform:uppercase; }
.ambient-title { margin-top:3px; color:#f5f5f7; font-size:14px; font-weight:600; }
.ambient-status { flex:0 0 auto; padding:4px 8px; border:1px solid rgba(255,255,255,.12); border-radius:999px; color:#a1a1aa; font-size:11px; white-space:nowrap; }
.ambient-status.active { color:#4ade80; border-color:rgba(74,222,128,.28); background:rgba(74,222,128,.07); }
.ambient-status.error { color:#fbbf24; border-color:rgba(251,191,36,.28); background:rgba(251,191,36,.07); }
.ambient-button { width:100%; min-height:42px; border:1px solid rgba(255,255,255,.14); border-radius:9px; background:rgba(255,255,255,.045); color:#f5f5f7; cursor:pointer; font:inherit; font-size:13px; font-weight:600; transition:transform .18s ease, background .18s ease, border-color .18s ease; }
.ambient-button:hover { transform:translateY(-1px); background:rgba(255,255,255,.075); border-color:rgba(255,255,255,.25); }
.ambient-button:disabled { cursor:default; opacity:.68; transform:none; }
.ambient-readout { display:flex; justify-content:space-between; gap:12px; margin-top:9px; color:#a1a1aa; font-size:11px; }
#ambient-classification { color:#f5f5f7; font-weight:600; text-align:right; }
.ambient-note { margin-top:8px; color:#71717a; font-size:10px; line-height:1.45; }
"""
...
export default function(component) {
  ...
  const button = root.querySelector("#ambient-authorize");
  const status = root.querySelector("#ambient-status");
  const level = root.querySelector("#ambient-level");
  const classification = root.querySelector("#ambient-classification");

  if (root.__ninoladesAmbientAudio) {
    root.__ninoladesAmbientAudio.sync(data || {});
    return () => {};
  }
  ...
  const classify = (value) => {
    if (!Number.isFinite(value)) return "Moderate Noise";
    if (value < 0.15) return "Quiet & Controlled";
    if (value <= 0.45) return "Moderate Noise";
    return "High Loudness / Busy Crowd";
  };

  const updateReadout = () => {
    const label = classify(amplitude);
    classification.textContent = label;
    if (!Number.isFinite(amplitude)) level.textContent = "Waiting for audio";
    else level.textContent = `Normalized RMS ${amplitude.toFixed(3)}`;
  };
  ...
  const publish = (force = false) => {
    ...
    setStateValue("snapshot", {
      amplitude: Number.isFinite(amplitude) ? Number(amplitude.toFixed(6)) : null,
      classification: classify(amplitude),
  ...
  const setStatus = (text, mode = "") => {
    ...
    status.className = `ambient-status ${mode}`.trim();
  };
  ...
  const startLoop = () => {
    ...
    const tick = () => {
      ...
      if (analyser && samples) {
        ...
        amplitude = Math.max(0, Math.min(1, rms));
        updateReadout();
        publish(false);
      }
  ...
  const authorize = async () => {
    ...
    level.textContent = "Waiting for browser permission";
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        throw new Error("This browser does not expose microphone access to web applications.");
      }
      ...
      button.textContent = "🎙️ Live Audio Tracking Authorized";
      updateReadout();
      publish(true);
      startLoop();
    } catch (error) {
      ...
      level.textContent = error?.message || "Microphone access unavailable";
      classification.textContent = "Moderate Noise";
      publish(true);
    } finally {
  ...
  button.addEventListener("click", authorize, { passive: true });
  ...
  const sync = (incoming) => {
    ...
    if (Number.isFinite(snapshot.amplitude)) {
      amplitude = Number(snapshot.amplitude);
      updateReadout();
    }
  };

  root.__ninoladesAmbientAudio = { sync };
  sync(data || {});

  return () => {
    // Keep the microphone stream alive across normal Streamlit reruns.
    // The browser releases it when the component is actually removed or the page closes.
  };
}
"""
...
class Event(Base):
    ...
    ambient_noise_classification = Column(String, nullable=True)
    ambient_noise_amplitude = Column(Float, nullable=True)
    ambient_noise_tracking_active = Column(Boolean, nullable=True)
...
# Live ambient-noise persistence migrations. These are additive and nullable so
# existing records remain compatible with the upgraded application.
for _table, _column, _sql_type in (
    ("events", "ambient_noise_classification", "VARCHAR"),
    ("events", "ambient_noise_amplitude", "FLOAT"),
    ("events", "ambient_noise_tracking_active", "BOOLEAN"),
    ("rapid_state_logs", "ambient_noise_classification", "VARCHAR"),
    ("rapid_state_logs", "ambient_noise_amplitude", "FLOAT"),
    ("rapid_state_logs", "ambient_noise_tracking_active", "BOOLEAN"),
    ("observations", "ambient_noise_classification", "VARCHAR"),
    ("observations", "ambient_noise_amplitude", "FLOAT"),
):
    try:
        with engine.connect() as conn:
            conn.execute(
                text(f"ALTER TABLE {_table} ADD COLUMN {_column} {_sql_type}")
            )
            conn.commit()
    except Exception:
        pass
...
def get_live_noise_snapshot() -> Dict[str, Any]:
    """Return the latest browser audio state, with a safe Moderate Noise fallback."""
    fallback = {
    ...
    state = st.session_state.get("ambient_audio_tracker")
    snapshot = None
    ...
    if not isinstance(snapshot, dict):
        return fallback
    authorized = bool(snapshot.get("authorized"))
    available = bool(snapshot.get("available"))
    try:
        amplitude = float(snapshot.get("amplitude"))
        if not math.isfinite(amplitude):
            raise ValueError
        amplitude = max(0.0, min(1.0, amplitude))
    except (TypeError, ValueError):
        amplitude = None
    if not authorized or not available or amplitude is None:
        return fallback
    return {
        "classification": classify_ambient_noise(amplitude),
        "amplitude": amplitude,
...
def safe_db_commit(db):
    """Commit atomically and roll back the unit of work if the transaction fails."""
    try:
        db.commit()
    except Exception:
        try:
...
def create_event(
    ...
    event = Event(
        ...
        ambient_noise_classification=noise["classification"],
        ambient_noise_amplitude=noise["amplitude"],
        ambient_noise_tracking_active=noise["tracking_active"],
    )
    try:
        db.add(event)
        safe_db_commit(db)
...
def update_event(
    ...
    noise = get_live_noise_snapshot()
    event.ambient_noise_classification = noise["classification"]
    event.ambient_noise_amplitude = noise["amplitude"]
    event.ambient_noise_tracking_active = noise["tracking_active"]
    try:
        safe_db_commit(db)
    except Exception:
        try:
...
def _delayed_timing_summary(interactions, df, timing):
    """Describe one delayed follow-up point, including actual response latency.

    Timeliness is reported as an observed deviation from the target interval:
    positive = recorded later than target; negative = recorded earlier.
    No arbitrary 'on-time' band is imposed, which avoids hiding real latency.
    """
    ...
    if latest.empty:
        return {
            ...
            "coverage_pct": None,
            "paired_n": {
                "curiosity": 0, "understanding": 0, "confidence": 0
            },
    ...
    for interaction_id, row in latest.iterrows():
        ...
        actual_hours = max(
            0.0, (recorded_ts - anchor_ts).total_seconds() / 3600.0
        )
        latency_rows.append({
            "interaction_id": interaction_id,
            "actual_hours": actual_hours,
            "deviation_hours": actual_hours - target_hours,
        })
    ...
    coverage_pct = (
        round(responses / participant_count * 100, 1)
        if participant_count else None
    )
...
def calculate_event_impact(db, event_id):
    ...
    if not delayed_all.empty:
        valid = delayed_all[delayed_all["follow_through"].notna()]
        if not valid.empty:
            follow_through_rate = round(
                float(valid["follow_through"].astype(bool).mean() * 100), 1
            )
...
# ============================================================
# 16B. LIVE AMBIENT AUDIO COMPONENT MOUNT
# ============================================================

ambient_audio_result = None
if streamlit_component_v2 is not None:
    ambient_audio_component = streamlit_component_v2(
        name="ninolades_live_ambient_audio",
        html=AMBIENT_AUDIO_HTML,
        css=AMBIENT_AUDIO_CSS,
        js=AMBIENT_AUDIO_JS,
    )
    _existing_noise_state = st.session_state.get("ambient_audio_tracker", {})
    _existing_snapshot = (
        _existing_noise_state.get("snapshot")
        if isinstance(_existing_noise_state, dict)
        else None
    )
    _default_noise_snapshot = _existing_snapshot or {
        "amplitude": None,
        "classification": "Moderate Noise",
        "authorized": False,
        "available": False,
    ...
    ambient_audio_result = ambient_audio_component(
        data={"snapshot": _default_noise_snapshot},
        default={"snapshot": _default_noise_snapshot},
        key="ambient_audio_tracker",
    ...
    components.html(
        """
        <div style="padding:14px 16px;border:1px solid rgba(255,255,255,.10);border-radius:14px;background:#111114;color:#f5f5f7;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Arial,sans-serif;">
          <div style="font-size:10px;letter-spacing:.12em;text-transform:uppercase;color:#8aa9ff;">LIVE AMBIENT AUDIO</div>
          <div style="margin-top:4px;font-size:14px;font-weight:600;">Browser live tracking requires a current Streamlit Components v2 release.</div>
          <div style="margin-top:6px;font-size:11px;color:#a1a1aa;line-height:1.45;">The safe classification fallback is Moderate Noise.</div>
        </div>
        """,
        height=105,
    )

live_noise = get_live_noise_snapshot()
live_noise_classification = live_noise["classification"]
...
# ============================================================
    ...
    if not events:
        ...
        with qc1:
            ...
            st.session_state["q_noise"] = live_noise_classification
            noise_slider_col, noise_status_col = st.columns([4.6, 2.2], vertical_alignment="center")
            ...
            with noise_status_col:
                if live_noise["tracking_active"]:
                    render_html('<div class="badge badge-success" style="margin-top:28px;">🎙️ Network Live Tracking Active</div>')
                else:
                    render_html('<div class="badge badge-warning" style="margin-top:28px;">🎙️ Network Live Tracking · Moderate fallback</div>')
        with qc2:
        ...
        render_ai_analysis_button(
            key="ai_analyse_outcome_predictor",
...
# ============================================================
    ...
    if not events:
        ...
        render_ai_analysis_button(
            key="ai_analyse_personality_predictor",
        ...
        if st.button("Log as New Participant", type="primary", use_container_width=True):
            ...
            try:
                ...
                try:
                    ...
                    pass
                st.error(f"Could not save the rapid state log: {exc}")
            else:
...
Environment: {event.environment}
Acoustic environment: {event.acoustic_environment}
Live ambient noise classification: {live_noise_classification}
Live ambient amplitude: {live_noise["amplitude"] if live_noise["amplitude"] is not None else "Unavailable"}
...
# ============================================================
    ...
    if not events:
        ...
        render_ai_analysis_button(
            key="ai_analyse_counterfactual_lab",
                ...
                with st.form(
                    ...
                    with c2:
                        understanding = st.slider(
                            ...
                            help="Use the same participant-reported or facilitator-administered 0–100 measure each time.",
                        )
                    with c3:
