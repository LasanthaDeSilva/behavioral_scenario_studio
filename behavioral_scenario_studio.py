...
class AIAnalysisResponse(BaseModel):
    ...
    uncertainty_and_limits: List[str] = Field(min_length=1, max_length=5)
    recommended_next_steps: List[str] = Field(min_length=2, max_length=6)
    safety_or_ethics_flags: List[str] = Field(min_length=1, max_length=5)
...
def generate_ai_analysis(client, model_name, context_title, context_data):
    """Generate a cautious, evidence-linked analysis of the current app state.

    This layer is deliberately interpretive: it does not replace deterministic
    calculations, does not diagnose people, and does not turn hypotheses into
    measurements. The returned structure forces evidence, uncertainty, and
    actionable next steps to remain separate.
    """
    prompt = f"""
CURRENT APP CONTEXT: {context_title}

AVAILABLE INFORMATION:
{context_data}

Analyze this information carefully and scientifically for an outreach
facilitator.

Requirements:
- Use only the information supplied above plus general scientific reasoning.
- Do not invent observations, measurements, participant traits, or outcomes.
- Clearly distinguish recorded/observed information from interpretation.
- Never diagnose, profile, or infer protected/sensitive characteristics.
- Do not claim causation from observational or cross-sectional data.
- Do not present AI-generated percentages as physiological, neurological,
  clinical, or experimentally established measurements.
- Treat participant-stated preferences as stronger evidence than inference.
- Prefer low-pressure, reversible, autonomy-preserving actions.
- "No intervention" is a valid next step when evidence does not justify action.
- Identify missing information that materially limits interpretation.
- Give practical next steps that could produce better evidence or improve
  clarity, accessibility, comfort, scientific understanding, or engagement.
- If the data are insufficient, explicitly say so rather than filling gaps.

Return:
1. A concise overall summary.
2. Key findings grounded in the supplied information.
3. The evidence basis for those findings.
4. Uncertainty and important limitations.
5. Concrete, proportionate next steps.
6. Any safety/ethics flags relevant to interpretation or action.
"""
    return run_gemini(
        client=client,
...
def render_ai_analysis_button(client, selected_model, title, context_data, key):
    """Render an additive AI Analyse control without changing existing outputs."""
    if st.button(
        "AI Analyse",
        key=f"ai_analyse_{key}",
        use_container_width=True,
        help="Ask Gemini for a cautious, evidence-linked interpretation and practical next steps. It does not replace deterministic measurements.",
    ):
        if client is None:
            st.error("Gemini is unavailable.")
        else:
            try:
                with st.spinner("AI is analysing the available evidence..."):
                ...
                st.session_state.setdefault("ai_analyses", {})[key] = result.model_dump()
            except Exception as exc:
                st.error(f"AI analysis failed: {exc}")

    result = st.session_state.get("ai_analyses", {}).get(key)
    if result:
        render_html("""
        <div class="premium-card" style="border-color:rgba(91,140,255,.25);">
            <div class="eyebrow">AI analysis · hypothesis layer</div>
            <div style="color:var(--text); line-height:1.75;">
        """)
        ...
        c1, c2 = st.columns(2)
        with c1:
            render_html('<div class="section-heading">Key findings</div>')
            for item in result["key_findings"]:
                st.markdown(f"- {item}")
            render_html('<div class="section-heading">Evidence basis</div>')
            for item in result["evidence_basis"]:
                st.markdown(f"- {item}")
        with c2:
            render_html('<div class="section-heading">Uncertainty & limits</div>')
            for item in result["uncertainty_and_limits"]:
                st.markdown(f"- {item}")
            render_html('<div class="section-heading">Safety / ethics</div>')
            for item in result["safety_or_ethics_flags"]:
                st.markdown(f"- {item}")

        render_html('<div class="section-heading">Recommended next steps</div>')
...
"""
    ...
    render_html('<div class="section-heading">AI design review</div>')
    render_ai_analysis_button(
        client,
        selected_model,
        "Experience Designer — current unsaved design",
        f"""
...
# ============================================================
    ...
    if not events:
        ...
        render_html('<div class="section-heading">AI review of prediction setup</div>')
        render_ai_analysis_button(
            client,
            selected_model,
            "Outcome Predictor — prediction inputs",
            f"""
...
Crowd details: {crowd_info}
Situational context: {situation_info}
Duration questionnaire: {duration_q}
Interaction density questionnaire: {interaction_density_q}
Environmental questionnaire parameters: {questionnaire_context}
""",
            "outcome_predictor_inputs",
        )
...
# ============================================================
    ...
    if not events:
        ...
        render_html('<div class="section-heading">AI review of personality scenario</div>')
        render_ai_analysis_button(
            client,
            selected_model,
            "Personality Predictor — selected scenario and profiles",
            f"""
Experience: {selected_event_name}
Selected profile names: {[p.name for p in selected_profiles]}
Profile descriptions and HEXACO/resilience values:
{[{
    "name": p.name,
    "description": p.description,
    "H": p.hexaco_h, "E": p.hexaco_e, "X": p.hexaco_x,
    "A": p.hexaco_a, "C": p.hexaco_c, "O": p.hexaco_o,
    "resilience": p.resilience_baseline
...
# ============================================================
    ...
    if not events:
        ...
        st.markdown("---")
        render_html('<div class="section-heading">AI review of live context</div>')
        render_ai_analysis_button(
            client,
            selected_model,
            "Live Copilot — current rapid-state context",
            f"""
Active experience: {event.name}
Objective: {event.objective}
Environment: {event.environment}
Sensory environment: {event.sensory_environment}
Acoustic environment: {event.acoustic_environment}
Current selected baseline label: {st.session_state.get("rapid_base", "not yet selected")}
Current selected reaction label: {st.session_state.get("rapid_state", "not yet selected")}
Use labels only as facilitator-recorded descriptors; do not treat them as clinical measurements.
""",
            "live_copilot_context",
        )
        ...
        if not st.session_state.active_interaction_id:
            ...
            render_html('<div class="section-heading">AI review of recorded observations</div>')
            recent_for_ai = get_recent_observations(db, interaction.id, limit=12)
            render_ai_analysis_button(
                client,
                selected_model,
                "Live Copilot — recorded observations",
                f"""
Experience: {event.name}
Participant code: {interaction.participant_code}
Current phase: {interaction.phase}
Participant-stated preference: {interaction.stated_preference or ""}
Recent observations:
{[
    {
    ...
    for o in reversed(recent_for_ai)
]}
""",
                "live_copilot_observations",
            )
...
# ============================================================
    ...
    if not events:
        ...
        if not logs:
            ...
            render_html('<div class="section-heading">AI reaction analysis</div>')
            render_ai_analysis_button(
                client,
                selected_model,
                "Scientific Reactions — rapid reaction records",
                df_logs.to_dict(orient="records"),
                f"scientific_reactions_{event.id}",
            )
...
# ============================================================
    ...
    if not events:
        ...
        if metrics is None:
            ...
            render_html('<div class="section-heading">AI decision-support review</div>')
            render_ai_analysis_button(
                client,
                selected_model,
                "Impact Observatory — deterministic longitudinal metrics",
                metrics,
                f"impact_observatory_{event.id}",
            )
...
# ============================================================
    ...
    if not events:
        ...
        render_html('<div class="section-heading">AI review of the proposed counterfactual</div>')
        render_ai_analysis_button(
            client,
            selected_model,
            "Counterfactual Lab — proposed change",
            f"""
Experience: {event.name}
Current design:
{design_description}
...
# ============================================================
    ...
    if not report_events:
        ...
        render_html('<div class="section-heading">AI review of report contents</div>')
        render_ai_analysis_button(
            client,
            selected_model,
            "PDF Report — selected program evidence and metrics",
            f"""
Program: {report_event.name}
Interactions: {len(report_interactions)}
Rapid-state records: {len(report_rapid_logs)}
Deterministic metrics:
{report_metrics}
Current stored AI/session analysis:
{report_session_snapshot}
""",
            f"pdf_report_{report_event.id}",
        )
