...
# Streamlit button state is an execution-time signal, not durable application state.
# Persisting these dynamic button keys causes StreamlitValueAssignmentNotAllowedError
# when a saved value is restored before the same widget is instantiated.
_EPHEMERAL_WIDGET_KEY_PREFIXES = (
    "edit_saved_event_",
    "del_prof_",
    "del_rlog_",
    "obs_btn_",
...
def _is_ephemeral_widget_key(key) -> bool:
    key_text = str(key)
    return any(key_text.startswith(prefix) for prefix in _EPHEMERAL_WIDGET_KEY_PREFIXES)
...
def _persistent_state_snapshot():
    ...
    for key, value in st.session_state.items():
        if (
            ...
            or _is_ephemeral_widget_key(key)
        ):
            continue
...
def restore_persistent_state(db, user_id: str):
    ...
    for key, value in payload.items():
        if (
            ...
            and not _is_ephemeral_widget_key(key)
        ):
            if key not in st.session_state:
                st.session_state[key] = value


def _remove_legacy_ephemeral_widget_state():
    """Remove button keys restored by older app versions before widgets render.

    This is a one-time migration for already-saved AppState records. It runs only
    for the known dynamic button-key prefixes and does not touch durable form/input
    state such as mem_*, phase_*, or pref_* values.
    """
...
# One-time migration: remove dynamic button values restored by older versions.
# These keys must never be treated as durable application state.
if "_legacy_ephemeral_widget_state_migrated" not in st.session_state:
    _remove_legacy_ephemeral_widget_state()
    st.session_state["_legacy_ephemeral_widget_state_migrated"] = True
    ...
    if not interactions:
        ...
        for idx, interaction in enumerate(interactions, 1):
            ...
            if surveys:
                ...
                survey_table = Table(
                    survey_rows,
                    colWidths=[24*mm, 22*mm, 26*mm, 23*mm, 27*mm, 62*mm],
                    repeatRows=1,
                    hAlign="LEFT",
                )
    ...
    if st.button(
        ...
        if not event_name.strip():
            ...
            try:
                ...
                st.session_state["experience_saved_selector"] = (
