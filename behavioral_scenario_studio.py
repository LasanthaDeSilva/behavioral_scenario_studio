...
# ============================================================
    ...
    if not events:
        ...
        event_map = {f"{e.name} ({e.date.strftime('%Y-%m-%d %H:%M')}) · {e.id[:8]}": e for e in events}
        ...
        profile_options = {p.id: p.name for p in saved_profiles}
        selected_profile_ids = st.multiselect(
            "Select Profiles to Test",
            options=list(profile_options.keys()),
            default=list(profile_options.keys())[:3], # Select up to 3 by default
        ...
        if st.button("Predict Personality Impacts", type="primary", use_container_width=True):
            if client is None:
                ...
                target_profiles = [p for p in saved_profiles if p.id in selected_profile_ids]
                profile_data_str = ""
